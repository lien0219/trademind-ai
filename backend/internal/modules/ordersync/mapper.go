...
func ToSyncedPayloads(in []platformp.PlatformOrder) []order.SyncedOrderPayload {
	...
	for _, po := range in {
		...
		out = append(out, order.SyncedOrderPayload{
			...
			DestinationCountryCode: strings.ToUpper(strings.TrimSpace(po.DestinationCountryCode)),
			DestinationRegion:      strings.TrimSpace(po.DestinationRegion),
			DestinationPostalCode:  strings.ToUpper(strings.TrimSpace(po.DestinationPostalCode)),
			Status:                 po.Status,
