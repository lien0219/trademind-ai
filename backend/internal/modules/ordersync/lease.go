...
func (s *Service) tryClaimOrderSyncTask(ctx context.Context, taskID uuid.UUID, workerID string, lease time.Duration) (*OrderSyncTask, bool, error) {
	if s == nil || s.DB == nil {
		return nil, false, fmt.Errorf("ordersync: no db")
	}
	now := time.Now().UTC()
	until := now.Add(lease)
...
func (s *Service) startOrderSyncLeaseRenewal(ctx context.Context, taskID uuid.UUID, workerID string, leaseTTL time.Duration) (stop func()) {
	if s == nil || s.DB == nil {
		return func() {}
	}
	interval := leaseTTL / 3
	...
	go func() {
		...
		for {
			...
			case <-tick.C:
				...
				_ = s.DB.WithContext(context.Background()).Model(&OrderSyncTask{}).
...
// RecoverLeaseExpired marks task failed for human retry (v1).
func (s *Service) RecoverLeaseExpired(ctx context.Context, taskID uuid.UUID) error {
	...
	if s.OpLog != nil {
		_ = s.OpLog.WriteBackground(ctx, operationlog.WriteOpts{
			...
			Action:      "order.sync.lease_expired",
			Resource:    "order_sync_task",
			ResourceID:  taskID.String(),
			Status:      "failed",
...
func (s *Service) RecoverLegacyRunning(ctx context.Context, taskID uuid.UUID, legacyCutoff time.Time) error {
	...
	if s.OpLog != nil {
		_ = s.OpLog.WriteBackground(ctx, operationlog.WriteOpts{
			...
			ResourceID:  taskID.String(),
			Status:      "failed",
			Message:     "legacy running order_sync_task recovered",
		})
	}
	return nil
}
