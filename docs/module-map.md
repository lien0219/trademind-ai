...
| 订单仓内履约 / 拣货波次 / 出库扫描复核 / 本地单据中心 | `backend/internal/modules/order/fulfillment_wave_*`、`backend/internal/modules/order/fulfillment.go`、`backend/internal/modules/order/router.go`、`backend/internal/database/migrate.go`、`admin/src/pages/Orders/FulfillmentWaves`、`admin/src/pages/Orders/FulfillmentPackVerification`、`admin/src/pages/Orders/FulfillmentDocuments`、`admin/src/pages/Orders/WarehouseAllocations`、`admin/src/services/fulfillmentWaves.ts`、路由/菜单权限、API 契约、后端测试、Admin 单元测试与 E2E、`docs/api.md`、`docs/ERP_ARCHITECTURE.md`；必须检查 tenant/store scope、同仓整单预占、不可变订单/规格及版本化单据快照、快照 hash、库位/条码绑定快照、订单/逐件商品/面单扫描与可选称重、完整数量校验、扫描审计不可变性、打印发起与重打原因审计、历史波次兼容、活动波次唯一归属、revision、动作与完成运行幂等、缺货阻断打包、部分完成持久化、直接履约旁路、取消保留预占、只读写请求安全以及 L0 下无官方承运商面单/运单申请/打印机控制、无真实物流/平台写、无 Worker/自动重试/跨仓拆单。 |
