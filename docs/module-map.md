...
| ERP 仓库 / 供应商 / 采购入库 | `backend/internal/modules/warehouse`、`supplier`、`procurement`、`inventory/warehouse_stock.go`、`admin/src/pages/Procurement`、`admin/src/services/procurement.ts`、`admin/config/routes.ts`、数据库迁移、`adminperm`、API 契约、Admin E2E、`docs/ERP_ARCHITECTURE.md`；必须检查 tenant scope、金额最小单位、状态机 revision、收货幂等、超收保护、同事务库存流水、权限/只读写请求安全和旧 `product_skus.stock` 兼容边界。 |
