...
| ERP 仓库 / 库存账 / 订单库存 / 供应商 / 采购入库退货 / 销售售后 | `backend/internal/modules/warehouse`、`supplier`、`procurement`、`salesreturn`、`inventory/warehouse_stock.go`、`inventory/warehouse_ledger.go`、`inventory/order_inventory.go`、`inventory/order_inventory_effect.go`、`inventory/warehouse_transfer.go`、`inventory/stocktake.go`、`order`、`ordersync`、`admin/src/pages/Procurement`、`admin/src/pages/SalesReturns`、`admin/src/pages/Inventory/WarehouseLedger`、`admin/src/pages/Inventory/WarehouseTransfers`、`admin/src/pages/Inventory/Stocktakes`、`admin/src/pages/Orders`、`admin/src/services/procurement.ts`、`admin/src/services/salesReturns.ts`、`admin/src/services/inventory.ts`、`admin/src/services/orders.ts`、`admin/config/routes.ts`、数据库迁移、`adminperm`、API 契约、Admin E2E、`docs/ERP_ARCHITECTURE.md`；必须检查 tenant scope、仓库必选/平台默认仓、单订单单仓、金额最小单位、状态机 revision、收货/退货/调库/订单库存幂等、采购原收货及销售原扣减的累计可退额度与取消释放、退货审批/执行或收货分权、良品/残次品可售语义、仅退款零库存写、盘点快照与差异过账、预占/释放/出库/回补事务、库存 effect 和 movement 不可变性、已有 effect 后订单行/仓库锁定、平台同步行 ID/数量保留、历史库存默认仓或待分配仓迁移、对账、物流包裹/事件租户隔离、事件键幂等与终态倒退保护、权限/只读写请求安全和旧 `product_skus.stock` 兼容边界。新增订单履约对账查询位于 `backend/internal/modules/order/fulfillment_reconciliation.go`，物流事件位于 `backend/internal/modules/order/tracking_events.go`，Admin 复用订单详情展示本地时间线并保留异常工作台入口，只读展示 matched/pending/mismatch/blocked。 |
