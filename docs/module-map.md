...
| 订单仓内履约 / 拣货波次 / 出库扫描复核 / 本地运费试算 / 本地单据中心 | `backend/internal/modules/logistics`、`backend/internal/modules/order/fulfillment_wave_*`、`backend/internal/modules/order/fulfillment.go`、`backend/internal/modules/order/router.go`、`backend/internal/database/migrate.go`、`admin/src/pages/Orders/LogisticsChannels`、`admin/src/pages/Orders/FulfillmentWaves`、`admin/src/pages/Orders/FulfillmentPackVerification`、`admin/src/pages/Orders/FulfillmentDocuments`、`admin/src/pages/Orders/WarehouseAllocations`、`admin/src/services/logistics.ts`、`admin/src/services/fulfillmentWaves.ts`、路由/菜单权限、API 契约、后端测试、Admin 单元测试与 E2E、`docs/api.md`、`docs/ERP_ARCHITECTURE.md`；必须检查 tenant/store scope、同仓整单预占、目的国家/区域/邮编快照、渠道与模板编码不可变、整数最小货币单位、重量区间和起始千克计费、费率模板 revision、显式试算与人工选择、不可变报价快照、报价后称重/承运商变化重新确认、不可变订单/规格及版本化单据快照、快照 hash、库位/条码绑定快照、订单/逐件商品/面单扫描、完整数量校验、扫描审计不可变性、打印发起与重打原因审计、历史波次兼容、活动波次唯一归属、revision、动作与完成运行幂等、缺货阻断打包、部分完成持久化、直接履约旁路、取消保留预占、只读写请求安全以及 L0 下无官方承运商面单/运单申请/打印机控制、无真实物流/平台写、无 Worker/自动重试/跨仓拆单。 |
...
| 订单预估利润 / 费用缺口 | `backend/internal/modules/profitability`、`backend/internal/modules/settlement` 的 matched 费用读取契约、`backend/internal/api/router.go`、`backend/internal/pkg/adminperm`、`admin/src/pages/Finance/OrderProfits`、`admin/src/services/profitability.ts`、Finance 路由/菜单、API 契约、Admin 单元测试与 E2E、`docs/api.md`、`docs/ERP_ARCHITECTURE.md`；必须检查 tenant/store scope、只读与导出权限分离、decimal 到整数最小货币单位的精确转换、JavaScript 安全整数、当前供应商价非历史成本、唯一有效供应商绑定、唯一活动波次运费事实、成功退款账实一致、仅 matched 平台费进入已知贡献额、未匹配费用为 `null`、已知贡献额与完整利润分离、公式版本 V2、派生状态/导出 5000 单上限、CSV 公式注入防护以及无自动分摊/真实结算/平台写/Worker。 |
