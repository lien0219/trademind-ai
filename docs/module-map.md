...
| 订单预估利润 / 费用缺口 | `backend/internal/modules/profitability`、`backend/internal/modules/order/cost_snapshot.go`、`backend/internal/modules/settlement` 的 matched 费用读取契约、`backend/internal/api/router.go`、`backend/internal/pkg/adminperm`、`admin/src/pages/Finance/OrderProfits`、`admin/src/services/profitability.ts`、Finance 路由/菜单、API 契约、Admin 单元测试与 E2E、`docs/api.md`、`docs/ERP_ARCHITECTURE.md`；必须检查 tenant/store scope、只读与导出权限分离、decimal 到整数最小货币单位的精确转换、JavaScript 安全整数、已履约订单只读不可变成本快照、历史快照缺失不回退当前目录、未履约目录价明确为估算、唯一有效供应商绑定、成本快照解析状态与来源时间、唯一活动波次运费事实、成功退款账实一致、仅 matched 平台费进入已知贡献额、未匹配费用为 `null`、已知贡献额与完整利润分离、公式版本 V3、派生状态/导出 5000 单上限、CSV 公式注入防护以及无 FIFO/移动加权/自动换汇/会计 COGS/自动分摊/真实结算/平台写/Worker。 |
