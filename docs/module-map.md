...
| 订单仓内履约 / 拣货波次 / 打包复核 | `backend/internal/modules/order/fulfillment_wave_*`、`backend/internal/modules/order/fulfillment.go`、`backend/internal/modules/order/router.go`、`backend/internal/database/migrate.go`、`admin/src/pages/Orders/FulfillmentWaves`、`admin/src/pages/Orders/WarehouseAllocations`、`admin/src/services/fulfillmentWaves.ts`、路由/菜单权限、API 契约、后端测试、Admin 单元测试与 E2E、`docs/api.md`、`docs/ERP_ARCHITECTURE.md`；必须检查 tenant/store scope、同仓整单预占、不可变订单/规格快照、活动波次唯一归属、revision、动作与完成运行幂等、缺货阻断打包、部分完成持久化、直接履约旁路、取消保留预占、只读写请求安全以及 L0 下无真实物流/平台写、无 Worker/自动重试/跨仓拆单。 |
