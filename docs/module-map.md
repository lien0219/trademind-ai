...
| ERP 仓库 / 库存账 / 供应商 / 采购入库 | `backend/internal/modules/warehouse`、`supplier`、`procurement`、`inventory/warehouse_stock.go`、`inventory/warehouse_ledger.go`、`admin/src/pages/Procurement`、`admin/src/pages/Inventory/WarehouseLedger`、`admin/src/services/procurement.ts`、`admin/src/services/inventory.ts`、`admin/config/routes.ts`、数据库迁移、`adminperm`、API 契约、Admin E2E、`docs/ERP_ARCHITECTURE.md`；必须检查 tenant scope、仓库必选、金额最小单位、状态机 revision、收货/调库幂等、超收保护、同事务仓库余额/流水/聚合投影、历史库存默认仓或待分配仓迁移、对账、权限/只读写请求安全和旧 `product_skus.stock` 兼容边界。 |
