...
**最后更新**：2026-05-16（**Shopee OAuth + 订单同步（beta）**：`internal/providers/platform/shopee/**`（**`SignHMAC`/公共与店铺签名**、**`ResolveRuntime`**：`platform_shopee.partner_id`、加密 **`partner_key`**、URL、timeout）；**OAuth**：**`GET /api/v1/shops/:id/oauth/shopee/authorize-url`**、**`POST .../oauth/shopee/callback`**（Redis **`state` 10 分钟**、`code` 换 token、**`shop_auth_tokens`**、**`shop.oauth.shopee.success|failed`**）；**Provider `beta`**（**`CapOrderSync`/`CapShopInfo`**，Registry **planned→beta**）；**`TestConnection`/`SyncOrders`** + **`mapping.go`→`PlatformOrder`**；沿用 **`ordersync`** + **`order.UpsertSyncedOrders`**；管理端 **`/shops`** Shopee 授权 Drawer；**`PUT /platform/settings/shopee`** 强 **`RuntimeFromMergedMap`** §3/§7/§8）
...
- **操作日志**：`operation_logs` 表；模块 `internal/modules/operationlog`；**`GET /api/v1/operation-logs`**（分页；**action / username / resource / start / end（RFC3339）** 筛选）。写入场景：**登录成功/失败**、**logout**、**settings 批量保存成功/失败**、**platform.settings.update**（Partner 应用配置；**不落明文密钥**）、**test-ai / test-storage 成功/失败**（消息不落敏感配置明文）；**采集**：…；**商品**：…；**AI 标题**：…；**AI 描述**：…；**内部订单**：**`order.create` / `order.update` / `order.delete`（软删）**、**`order.item.{create|update|delete}`**、**`order.shipment.{create|update|delete}`**；**订单同步任务**：**`order.sync.create` / `order.sync.running` / `order.sync.success` / `order.sync.failed` / `order.sync.retry`**（**不落 token / 不全量 raw_data**）；**店铺 OAuth**：**`shop.oauth.tiktok.success|failed`、`shop.oauth.shopee.success|failed`**（不落 token）；**客服（MVP）**：**`customer.conversation.create` / `customer.conversation.update` / `customer.conversation.close`（含软删会话）**、**`customer.conversation.link_order`**、**`customer.message.create`**、**`customer.conversation.replied`**、**`customer.reply_generate.success` / `customer.reply_generate.failed`**、**`customer.reply_suggestion.edit` / `accept` / `discard`**（**不落客户正文全量**；可含 **conversationId / suggestionId / platform / 长度摘要**）；**图片任务**：…
...
- **统一店铺（平台授权基座，MVP）**：模块 **`internal/modules/shop`**；表 **`shops`**、**`shop_auth_tokens`**（敏感值 **AES-GCM**，API **脱敏 + masked 不覆盖**）。**`internal/providers/platform`**：**`Registry`** + **`manual` / `mock` / `planned`（各带 `AppConfigSchema` 预置）**。**TikTok Shop**、**Shopee** 在 **`api/router`** 分别 **`tiktok` / `shopee`** 绑定 **`BindShops` + `RegisterProvider()`**，状态 **`beta`**，能力 **`CapOrderSync`/`CapShopInfo`**（Shopee 不向 UI 宣称商品/客服/库存等已实现能力）。**应用级**：**`GET /api/v1/platform/providers`** 返回 **`appConfigSchema`/`settingsGroupKey`**；**`GET`/`PUT /api/v1/platform/settings/:platform`** 写入 **`platform_tiktok` / `platform_shopee`** 等分组，其中 **`tiktok.beta`/`shopee.beta`** **`PUT` 前**做 **`RuntimeFromMergedMap`** 完整性校验，**`planned`** 仍允许部分保存；**敏感项加密**，**`platform.settings.update`** 审计；**access/refresh token** 仅落 **`shop_auth_tokens`**。**店铺级**：Drawer **可选**覆盖 **App Key/Secret（TikTok）或 Partner ID/Key（Shopee）/ redirect**。**OAuth（Redis `state`，10 分钟）**：**`/shops/:id/oauth/tiktok/*`**、**`/shops/:id/oauth/shopee/*`**（callback 需 **`shopId`**）；操作日志 **`shop.oauth.tiktok.*` / `shop.oauth.shopee.*`**。**`tiktok.ResolveRuntime` / `shopee.ResolveRuntime`** 合并 **settings 小写键 + 店铺覆盖**，不写死密钥；缺 Partner 报错 **`platform config incomplete … platform_tiktok…`** / **`…platform_shopee.partner_id…`**。**`shop.*` 不写密钥**。与订单：**`shopSummary`**。
- **平台订单同步（框架）**：模块 **`internal/modules/ordersync`**；表 **`order_sync_tasks`**（UUID、`shop_id`、`platform`、`task_type=order_sync`、状态、**JSONB `input`/`output`**，**不落 token**）；**Redis `ORDER_SYNC_QUEUE_*`** + **Worker**（**`ORDER_SYNC_QUEUE_ENABLED=false`** 时同步执行）；**编排**：校验 **`CapOrderSync`**／店铺 **`active`**；**`tiktok`、`shopee`（均为 beta）** 走 **`SyncOrders`**；**`planned`/`manual`** **501／不支持**；**`mock`** 模拟。**API**：**`POST /api/v1/shops/:id/sync-orders`**、**`GET /api/v1/order-sync/tasks*`**。**`order.UpsertSyncedOrders`** 入库 **`orders`/`order_items`/`order_shipments`**；**`/health` `orderSyncQueue`**。**管理端**：**`/orders/sync-tasks`**、**`/shops` 同步**；**`/orders`** **platform/externalOrderId**；前端不直连平台。
...
- **店铺管理（授权基座）**：路由 **`/shops`**；**`services/shops.ts`** + **`platformOpen.ts`**：**TikTok / Shopee OAuth**；默认 **不写每店 Partner 密钥**，引导 **「设置 → 平台开放配置」**；**`formatPlatformPartnerErr`** 映射不完整 **`platform_*`** 配置；**创建店铺 / 授权 Drawer** 前校验 **`GET /api/v1/platform/settings/:platform`** 必填（**`****` 视同已配置密钥**）；生成授权链接在无配置时 **warning**。**同步订单**、**`/orders/sync-tasks`**。**`SHOP_*` / `PLATFORM_PROVIDER_STATUS`**（**`beta`** 前端显示 **测试中**）。
...
19. **AI 客服与内部订单上下文**：工作台已支持 **绑定内部订单**（含 **同步入库的 `orders`**）；**仍无** 各平台 **实时客服消息收发** / **WebSocket**。**TikTok Shop（beta）与 Shopee（beta）订单** 均可走 **`SyncOrders`** 入库；**客服外发与消息拉取** 未接。
...
25. **TikTok Shop 与 Shopee（beta）订单同步**：均已接 **`providers/platform/*/SyncOrders`** → **`order.UpsertSyncedOrders`** / **`order_sync_tasks`**；生产需关注分页、字段与 **`cursor`** 演进。
26. **Shopee 商品刊登未完成**。
27. **Shopee 客服消息拉取 / 人工发送未完成**。
28. **Shopee 库存同步未完成**。
29. **Lazada / Amazon 等平台订单同步未完成**。
...
1. **TikTok Shop 深化**：生产实测分页与字段；按需补 **`docs/platform-provider.md` / `api.md`**。
2. **Shopee 深化**：生产实测 **`get_order_list` / `get_order_detail`** 与状态映射；按需补文档。
3. **Lazada OAuth + 订单同步**（或 Amazon 择一）。
4. **Shopee 客服消息拉取 / 人工发送**（延续不外发默认）。
5. **平台商品刊登**（多平台，与订单管道解耦）。
6. **多实例 Worker heartbeat / 租约**：采集 / 图片 / **订单同步**。
7. **Collector 演进**：同前（AliExpress、自定义、新源、反爬）。
