...
| 维度 | 状态 |
|------|------|
| **路线图阶段** | **第 5 阶段（采集）**保持；**第 6 阶段（AI 图片）**：**remove.bg** + **OpenAI Image `generate_scene`** + **ComfyUI `generate_scene` / 基础 `replace_background`** 已与 **`image_tasks` + Storage + `files` + Worker + 自动退避重试（`retrying` + scheduler）** 贯通；**`openai_image` `replace_background` 仍为占位错误**（见 §7） |
...
- **AI 图片任务**：模块 **`internal/modules/imagetask`**；表 **`image_tasks`**（**`task_type`**：`remove_background` / …；**`status`**：**`pending` / `running` / `retrying` / `success` / `failed` / `cancelled`**；**`retry_count` / `max_retries` / `next_retry_at` / `retry_enqueued_at`**；**JSONB `input` / `output`**；**`source_image_id`** → **`files` / `product_images`** 解析 **`public_url`**；**`result_file_id` / `result_url`**；**不落库图片二进制**）。**Image Provider**：**`internal/providers/image`**：**`noop`**；**`removebg`**；**`openaiimage`**；**`comfyui`**（**`POST /prompt`、`/history`、`/view`、`/upload/image`**；**日志不打 API Key / 完整 workflow**）；**`factory.NewForTask`：`noop` | `removebg` | `openai_image` | `comfyui`**，读 **`settings.image`**（密钥 **解密不写日志**，**不回退 `settings.ai.api_key`**）。**`remove_background`**：**强制 `provider=removebg`**。**`generate_scene` + `openai_image`**：**`prepareGenerateSceneHints` → `assembled_prompt`**；**+ `comfyui`**：同 **hints** + **模板变量**。**`generate_scene`**：**`openai_image` / `comfyui` 可无源图**。**`replace_background`**：**`comfyui`** 基础链路（须 **workflow + output 节点**；**`image` 节点** 用于上行源图；**`openai_image` 仍未实现**）。**`IMAGE_QUEUE_ENABLED` + Redis**、**Worker**（认领 **`pending`** 或已调度入队的 **`retrying`**，条件 **`next_retry_at IS NULL`**）、**503 回滚**、**`GET /api/v1/image/tasks/monitor`**（**`retry` / `recentRetrying` / `recentFailures`**）、**人工 retry**；**`IMAGE_AUTO_RETRY_ENABLED`**（**.env 默认 true**；**`IMAGE_MAX_RETRIES` / `IMAGE_RETRY_*_DELAY`**）与 **`StartImageRetryScheduler`**（约 **5s**、到期 **CAS** **`LPUSH`** **`image:tasks`**）；可重试错误 **`IsRetryableImageTaskError`**（**5xx** / **429** / 超时 / 网络类；**缺 Key、workflow/JSON、不可达公网源图、`not implemented` 等**不重试）；操作日志 **`image.task.create` / `retry` / `success` / `failed` / `auto_retry_scheduled` / `auto_retry_enqueued` / `retry_exhausted`**（**不写密钥与完整 workflow**）；**Comfy 成功 `output`**：**`promptId`/`workflow`（空）** 等；**执行超时** 对 **`comfyui`** 不低于 **`comfyui_max_poll_seconds` + `comfyui_timeout_sec`**，再与 **`IMAGE_TASK_TIMEOUT_SECONDS`** 取 cap。**管理端**：**`/settings/image`**（**ComfyUI 大文本 workflow**）、**`/ai/image-tasks`**、**商品详情图片 Tab**（**`replace_background` / `comfyui`**）。**其它**：noop **resize/enhance**；removebg **仅 remove_background**；openai **仅 generate_scene**。
...
- [x] **AI 图片任务**：**remove.bg** + **OpenAI Image** + **ComfyUI**（**`generate_scene` / 基础 `replace_background`**）+ **Redis Worker** + **自动退避重试（`retrying` + scheduler）**（详见 §3.2）；**`openai_image` `replace_background` 仍为占位错误**。**未完成**：**本地非公网 multipart→remove.bg**、**OpenAI 图像编辑 `replace_background`**
...
| AI 图片 | **`internal/providers/image`**：**`noop`** + **`removebg`** + **`openaiimage`** + **`comfyui`**（HTTP REST，**结果统一 PNG**）；**factory**：**`noop` \| `removebg` \| `openai_image` \| `comfyui`**；**异步队列 + 自动退避重试**同 §3.2（**`IMAGE_AUTO_RETRY_*`**）；**OpenAI Image 密钥与 `settings.ai.api_key` 分离（不回退）**；**OpenAI `replace_background` 未实现**；**ComfyUI `replace_background`** 为 **可配置 workflow 的基础链路**；**未完**：multipart remove.bg、COS/OSS Provider |
...
17. **AI 图片（剩余）**：**`provider=openai_image` `replace_background` 仍为「未实现」**；**ComfyUI `replace_background`** 依赖用户 **API 工作流**（**非通用抠图/合成** guarantee）；**本地非公网源图 multipart 上传 remove.bg**。
18. **AI 客服**：**未完成**。
...
1. **本地非公网图 multipart** 直传 remove.bg（当前抠图仍依赖 **源图 URL 公网可达**）。
