...
**最后更新**：2026-05-17（**Lazada OAuth + 订单同步（beta）**：`internal/providers/platform/lazada/**`（**`Sign`（HMAC-SHA256，`sign_method=sha256`）**、**`ResolveRuntime`**：`platform_lazada.app_key`、加密 **`app_secret`**、**`auth_base_url`/`api_base_url`**、timeout）；**OAuth**：**`GET /api/v1/shops/:id/oauth/lazada/authorize-url`**、**`POST .../oauth/lazada/callback`**（Redis **`state` 10 分钟**、`code` 换 token、**`shop_auth_tokens`**、**`shop.oauth.lazada.success|failed`**）；**Provider `beta`**（**`CapOrderSync`/`CapShopInfo`**，**bootstrap 移除 Lazada planned 占位**，**`RegisterProvider` 覆盖注册**）；**`TestConnection`（`/seller/get`）/`SyncOrders`（`/orders/get`…）** + **`mapping.go`→`PlatformOrder`**；沿用 **`ordersync`** + **`order.UpsertSyncedOrders`**；管理端 **`/shops`** Lazada 授权 Drawer；**`PUT /platform/settings/lazada`** 强 **`RuntimeFromMergedMap`** §3/§7/§8）
...
31. **Shopee 库存同步未完成**。
32. **Amazon 订单同步未完成**。
33. **多实例 Worker heartbeat / 任务租约**：订单同步 Worker 仍为进程内 **`workerRunning` 标记**（与采集、图片 Worker 同类遗留）。
...
4. **多实例 Worker heartbeat / 租约**：采集 / 图片 / **订单同步**。
5. **TikTok / Shopee / Lazada 生产实测**：分页、字段、沙箱与 **`cursor`/offset**；按需补 **`docs/platform-provider.md` / `api.md`**。
6. **Collector 演进**：同前（AliExpress、自定义、新源、反爬）。
