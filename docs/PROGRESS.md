...
**最后更新**：2026-05-16（**remove.bg 非公网源图**：Storage **`Get`（local + S3/R2/MinIO）**、`internal/modules/imagetask/source_resolver` 统一解析 **`sourceImageId`/`sourceImageUrl`**；**能读则 multipart `image_file`**，否则 **`httppublic.IsPublicHTTPURL` 为真时走 `image_url`**；**`/static/...` 本地键映射**（`settings.kind=local` 或 `files.storage_kind=local`）；**`IsRetryableImageTaskError`** 将 **`source image is not readable and not publicly accessible`** / **`unsupported storage provider`** / **`invalid object key`** / **`file not found`** 等判为 **不可自动重试**；管理端 **`/ai/image-tasks`** 增加可选 **`sourceImageId`** 与文案。风险：**`public_base` 若指向内网域名，`IsPublicHTTPURL` 仍可能视为公网**（仅按 host 字面判断、无 DNS），见 §7。）
...
| 维度 | 状态 |
|------|------|
| **路线图阶段** | **第 5 阶段（采集）**保持；**第 6 阶段（AI 图片）**：**remove_background** 已支持 **非公网源图后端直传 remove.bg（`image_file`）** + **公网 `image_url`**；**OpenAI Image `generate_scene`** + **ComfyUI `generate_scene` / 基础 `replace_background`** 已与 **`image_tasks` + Storage + Worker + 自动重试** 贯通；**`openai_image` `replace_background` 仍为占位错误**（见 §7） |
...
- [x] **AI 图片任务**：**remove.bg `image_file` + `image_url`** + **OpenAI Image** + **ComfyUI**（**`generate_scene` / 基础 `replace_background`**）+ **Redis Worker** + **自动退避重试**（详见 §3.2）；**`openai_image` `replace_background` 仍为占位**。**未完成**：**OpenAI 图像编辑 `replace_background`**
...
| AI 图片 | **`internal/providers/image`**：**`noop`** + **`removebg`**（**`image_file` / `image_url`**）+ **`openaiimage`** + **`comfyui`**（HTTP REST，**结果统一 PNG**）；**factory**：**`noop` \| `removebg` \| `openai_image` \| `comfyui`**；**异步队列 + 自动退避重试**同 §3.2（**`IMAGE_AUTO_RETRY_*`**）；**源解析**：**`imagetask/source_resolver`** + **`httppublic.IsPublicHTTPURL`**；**OpenAI Image 密钥与 `settings.ai.api_key` 分离（不回退）**；**OpenAI `replace_background` 未实现**；**ComfyUI `replace_background`** 为 **可配置 workflow 的基础链路**；**COS/OSS Storage `Get` 未实现** |
...
17. **AI 图片（剩余）**：**`provider=openai_image` `replace_background` 仍为「未实现」**；**ComfyUI `replace_background`** 依赖用户 **API 工作流**（**非通用抠图/合成** guarantee）。
...
1. **OpenAI Image**：补足 **`replace_background`（multipart 图像编辑）** 若产品需要；Flux / 即梦 等 Provider 预留。
2. **COS / OSS 独立 Provider**：在 **腾讯云 / 阿里云** 原生 SDK（或专有域名）上与 **`files`/设置页**对齐，而不走 S3 兼容端点。
3. **AI 客服**：建议回复生成 + 人工确认发送（MVP 不自动发）。
