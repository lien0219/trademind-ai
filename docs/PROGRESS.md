...
| 维度 | 状态 |
|------|------|
| **路线图阶段** | **第 5 阶段（采集）**保持；**第 6 阶段（AI 图片）**：**remove_background**（**remove.bg `image_file`/`image_url`**）；**OpenAI Image **`generate_scene` + `replace_background`（`POST /images/edits` multipart `image[]`）**；**ComfyUI **`generate_scene` / `replace_background`**；均已接入 **`image_tasks` + Storage + Redis Worker + 自动重试** |
