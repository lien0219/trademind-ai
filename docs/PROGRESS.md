...
**最后更新**：2026-05-16（**ComfyUI + OpenAI Image（图片）**：**`internal/providers/image/comfyui`** — **`POST /prompt`**、轮询 **`/history/{prompt_id}`**、**`/view`** 下载输出，**`/upload/image`** 上传源图；**结果解码并统一 PNG** 后 **`files.SaveProcessed`**（**`comfyui-<taskId>.png`**，**`image-tasks/YYYYMMDD/<taskId>-comfyui.png`**）；**`settings.image`**：**`provider`** 含 **`comfyui`**；**`comfyui_base_url`（默认 `http://127.0.0.1:8188`）**、**`comfyui_api_key`（可选，加密）**、**`comfyui_workflow_json`（json；空/`{}` 报错）**、**`comfyui_prompt_node_id` / `comfyui_image_node_id` / `comfyui_output_node_id`**、**`comfyui_timeout_sec`（180）**、**`comfyui_poll_interval_seconds`（2）**、**`comfyui_max_poll_seconds`（180）**；**workflow 占位符** **`{{prompt}}`（优先组装 `assembled_prompt`）**、`{{negativePrompt}}`、`{{sourceImageUrl}}`、`{{productTitle}}`、`{{scene}}`、`{{style}}`、`{{background}}`、`{{platform}}`；**`generate_scene`**：**`openai_image` 不变**；**`comfyui`** 走相同 **`assembled_prompt`/`productTitle` 增强**；**`replace_background`**：**`comfyui`** 基础链路（缺 image+output 节点或未配置 workflow → **明确错误**；**`prepareReplaceBackgroundHints`**）；**`openai_image` `replace_background` 仍未实现**；**`remove_background` 仍强制 `removebg`**；**成功 `output`** 含 **`promptId`/`workflow`（空字符串占位）** 等；**执行 context** 对 Comfy 取 **`max_poll + http`** 与 **`IMAGE_TASK_TIMEOUT_SECONDS`** 的较小上限；**管理端**：**`/settings/image`**、**`/ai/image-tasks`（comfyui、replace_background 表单）**、**商品详情**：**comfyui / replace_background / 无源场景图**。**依赖** **`golang.org/x/image/webp`**。**遗留**：**图片自动退避重试**、**本地非公网 multipart→remove.bg**、**OpenAI replace_background**、**AI 客服** 等，见 §7–§8。）
...
| 维度 | 状态 |
|------|------|
| **路线图阶段** | **第 5 阶段（采集）**保持；**第 6 阶段（AI 图片）**：**remove.bg** + **OpenAI Image `generate_scene`** + **ComfyUI `generate_scene` / 基础 `replace_background`** 已与 **`image_tasks` + Storage + `files` + Worker** 贯通；**`openai_image` `replace_background` 仍为占位错误**（见 §7） |
...
- **迁移**：启动时 `database.AutoMigrate` — `admin_users`、`settings`、**`operation_logs`**、**`files`**、**`products`、`product_images`、`product_skus`、`image_tasks`、`collect_batches`、`collect_tasks`、`collect_task_events`**、**`ai_prompts`、`ai_tasks`**；启动后 **`aiprompt.EnsureDefaults`** 写入默认 **`product_title_optimize`**、**`product_description_generate`**（各仅在缺失时插入）；**`settings.EnsureImageDefaults`** 写入 **`image` 分组**默认键（**`provider=noop`**；**`removebg_*`**；**`openai_image_*`**；**`comfyui_base_url`（`http://127.0.0.1:8188`）**、**`comfyui_api_key`（加密，可选）**、**`comfyui_workflow_json`（json，空）**、**`comfyui_prompt_node_id` / `comfyui_image_node_id` / `comfyui_output_node_id`**、**`comfyui_timeout_sec`（180）**、**`comfyui_poll_interval_seconds`（2）**、**`comfyui_max_poll_seconds`（180）**；**`timeout_sec`**；**仅缺失键插入**）
...
- **AI 图片任务**：模块 **`internal/modules/imagetask`**；表 **`image_tasks`**（**`task_type`**：`remove_background` / …；**`status`**：**`pending` / `running` / `success` / `failed` / `cancelled`**；**JSONB `input` / `output`**；**`source_image_id`** → **`files` / `product_images`** 解析 **`public_url`**；**`result_file_id` / `result_url`**；**不落库图片二进制**）。**Image Provider**：**`internal/providers/image`**：**`noop`**；**`removebg`**；**`openaiimage`**；**`comfyui`**（**`POST /prompt`、`/history`、`/view`、`/upload/image`**；**日志不打 API Key / 完整 workflow**）；**`factory.NewForTask`：`noop` | `removebg` | `openai_image` | `comfyui`**，读 **`settings.image`**（密钥 **解密不写日志**，**不回退 `settings.ai.api_key`**）。**`remove_background`**：**强制 `provider=removebg`**。**`generate_scene` + `openai_image`**：**`prepareGenerateSceneHints` → `assembled_prompt`**；**+ `comfyui`**：同 **hints** + **模板变量**。**`generate_scene`**：**`openai_image` / `comfyui` 可无源图**。**`replace_background`**：**`comfyui`** 基础链路（须 **workflow + output 节点**；**`image` 节点** 用于上行源图；**`openai_image` 仍未实现**）。**异步队列**、**Worker**、**503 回滚**、**monitor**、**retry**、操作日志 **`image.task.*`** 同前；**Comfy 成功 `output`**：**`promptId`/`workflow`（空）** 等；**执行超时** 对 **`comfyui`** 不低于 **`comfyui_max_poll_seconds` + `comfyui_timeout_sec`**，再与 **`IMAGE_TASK_TIMEOUT_SECONDS`** 取 cap。**管理端**：**`/settings/image`**（**ComfyUI 大文本 workflow**）、**`/ai/image-tasks`**、**商品详情图片 Tab**（**`replace_background` / `comfyui`**）。**其它**：noop **resize/enhance**；removebg **仅 remove_background**；openai **仅 generate_scene**。
...
- **商品草稿**：路由 **`/product/drafts`**，`ProTable` → **`GET /api/v1/products`**；**`/product/drafts/:id`** **Tabs**（基础、AI 标题/描述、**图片管理**（上传、`createProductImage`、**AI 图片任务**：**resize/noop**、**remove_background**、**replace_background（comfyui）**、**generate_scene（openai_image / comfyui）**、Prompt/背景/style、**可无源场景图**、异步提示 + **`/ai/image-tasks`**、reorder）、**SKU 表**、最近 AI 任务）；**`/ai/prompts`**、**`/ai/tasks`**、**`/ai/image-tasks`**（~4s 轮询）。**`products.ts` / `imageTasks.ts`** 封装 API。
...
- [x] **AI 图片任务**：**remove.bg** + **OpenAI Image** + **ComfyUI**（**`generate_scene` / 基础 `replace_background`**）+ **Redis Worker**（详见 §3.2）；**`openai_image` `replace_background` 仍为占位错误**。**未完成**：**图片自动重试 scheduler**、**本地非公网 multipart→remove.bg**、**OpenAI 图像编辑 `replace_background`**
...
| AI 图片 | **`internal/providers/image`**：**`noop`** + **`removebg`** + **`openaiimage`** + **`comfyui`**（HTTP REST，**结果统一 PNG**）；**factory**：**`noop` \| `removebg` \| `openai_image` \| `comfyui`**；**异步队列**同 §3.2；**OpenAI Image 密钥与 `settings.ai.api_key` 分离（不回退）**；**OpenAI `replace_background` 未实现**；**ComfyUI `replace_background`** 为 **可配置 workflow 的基础链路**；**未完**：multipart remove.bg、scheduler 自动重试 |
...
17. **AI 图片（剩余）**：**`provider=openai_image` `replace_background` 仍为「未实现」**；**ComfyUI `replace_background`** 依赖用户 **API 工作流**（**非通用抠图/合成** guarantee）；**本地非公网源图 multipart 上传 remove.bg**；**`IMAGE_AUTO_RETRY_*` scheduler** 仍为占位。
18. **AI 客服**：**未完成**。
...
1. **图片任务增强**：**自动退避重试**（落实 **`IMAGE_AUTO_RETRY_*`**）；**本地非公网图 multipart** 直传 remove.bg。
2. **OpenAI Image**：补足 **`replace_background`（multipart 图像编辑）** 若产品需要；Flux / 即梦 等 Provider 预留。
