...
**最后更新**：2026-05-16（**平台订单同步基础框架（无真实 TikTok/Shopee/Lazada/Amazon API）**：表 **`order_sync_tasks`**；模块 **`internal/modules/ordersync`** + Redis 队列 **`ORDER_SYNC_*`** + **Worker**；API **`POST /api/v1/shops/:id/sync-orders`**、**`GET /api/v1/order-sync/tasks`**（筛选 **shopId/platform/status/start/end**）、**`GET …/tasks/:id`**、**`POST …/tasks/:id/retry`**；**`internal/providers/platform`** 扩展 **`OrderSyncProvider`** 与 **`PlatformOrder`/`PlatformOrderItem`/`PlatformShipment` 中立模型**（**`manual`** **不支持**、**`mock`** **生成固定 external_order_id 模拟单**、**`planned`** **`platform order sync not implemented`**）；**`order.UpsertSyncedOrders`** 幂等写入 **`orders`/`order_items`/`order_shipments`**（**不覆盖手工 `remark`**，`raw_data` 仅收敛摘要）；**`/health`** **`orderSyncQueue`**；操作日志 **`order.sync.*`**；管理端 **`/orders/sync-tasks`**、**`/shops` 同步订单 Modal**。此前主线仍包含：**统一 `shops`/`shop_auth_tokens`**、采集 **custom `beta`**、**AliExpress `beta`** 等 — 详见 §3。）
...
- **操作日志**：`operation_logs` 表；模块 `internal/modules/operationlog`；**`GET /api/v1/operation-logs`**（分页；**action / username / resource / start / end（RFC3339）** 筛选）。写入场景：**登录成功/失败**、**logout**、**settings 批量保存成功/失败**、**test-ai / test-storage 成功/失败**（消息不落敏感配置明文）；**采集**：…；**商品**：…；**AI 标题**：…；**AI 描述**：…；**内部订单**：**`order.create` / `order.update` / `order.delete`（软删）**、**`order.item.{create|update|delete}`**、**`order.shipment.{create|update|delete}`**；**订单同步任务**：**`order.sync.create` / `order.sync.running` / `order.sync.success` / `order.sync.failed` / `order.sync.retry`**（**不落 token / 不全量 raw_data**）；**客服（MVP）**：**`customer.conversation.create` / `customer.conversation.update` / `customer.conversation.close`（含软删会话）**、**`customer.conversation.link_order`**、**`customer.message.create`**、**`customer.conversation.replied`**、**`customer.reply_generate.success` / `customer.reply_generate.failed`**、**`customer.reply_suggestion.edit` / `accept` / `discard`**（**不落客户正文全量**；可含 **conversationId / suggestionId / platform / 长度摘要**）；**图片任务**：…
...
- **统一店铺（平台授权基座，MVP）**：模块 **`internal/modules/shop`**；表 **`shops`**（**`tenant_id`、平台键、`shop_name`、状态/授权状态、地区币种时区等**）、**`shop_auth_tokens`**（**`auth_type`、各平台口令/Token 字段 JSONB + 可空 `expires_at`**，敏感值 **AES-GCM**、API **脱敏 + `****` masked 不覆盖**）；**`internal/providers/platform`**：**`Registry`** + **`manual` / `mock` / `planned`** 内置实现（**无 import cycle**；**真实 OAuth 不接**）。**JWT API**：**`GET /api/v1/platform/providers`**；**`GET/POST /api/v1/shops`**、**`GET/PUT/DELETE /api/v1/shops/:id`**、**`PUT /api/v1/shops/:id/auth`**、**`POST /api/v1/shops/:id/test-connection`**。**`PlainAuthForProviderCtx`**：Worker 解密 **`shop_auth_tokens`** 供 **`SyncOrders`**（**不写日志**）。操作日志：**`shop.*`**（创建/更新/删/授权/测试 等，**不写密钥明文**）。**与订单/客服**：**`order.Service`、`customerchat.Service`** 注入 **`Shops`**，列表与详情附带 **`shopSummary`**。
- **平台订单同步（框架）**：模块 **`internal/modules/ordersync`**；表 **`order_sync_tasks`**（**UUID**、`shop_id`、`platform` 冗余、`task_type=order_sync`、`status`、`mode`、`cursor`、计数与 **JSONB `input`/`output`**，**不落 token**）；**Redis `ORDER_SYNC_QUEUE_*`** + **`ordersync.StartWorker`**（**`ORDER_SYNC_QUEUE_ENABLED=false`** 时 **API 进程内同步执行 `ProcessQueuedTask`**）；**编排**：校验 **`CapOrderSync`** / 店铺 **`active`** / **`planned`→501 语义错误文案** / **`manual`→不支持** → 创建任务 → **LPUSH** 或 **inline** → **`OrderSyncProvider.SyncOrders`** → **`order.UpsertSyncedOrders`**。**前端**不直连平台 API；**`/orders`** 列表支持 **`platform` 筛选**与 **`externalOrderId` 列**。
...
- **店铺管理（授权基座）**：路由 **`/shops`**（侧栏 **店铺 → 店铺管理**）；**`services/shops.ts`**：**平台 Provider 列表**、店铺 CRUD、**授权表单（敏感项脱敏）**、**测试连接**、**同步订单 Modal（mock 可走通；planned/manual 前端拦截）**、**同步记录**链接；常量 **`SHOP_*` / `PLATFORM_PROVIDER_STATUS`**（`status.ts`）。
...
19. **AI 客服与内部订单上下文**：工作台已支持 **绑定内部订单**（含 **平台同步入库的 `orders`**，上下文不变）；**仍无** TikTok/Shopee 等平台 **实时消息收发** / **客服消息自动拉取或外发**，**无 WebSocket**。**真实平台订单同步**尚未对接（当前仅 **Provider 框架 + `mock` 联调**）。
...
25. **未实现真实 TikTok 订单同步**（需 OAuth + 开放平台订单 API）。
26. **未实现真实 Shopee 订单同步**（需 OAuth + 订单 API）。
27. **未实现真实 Lazada / Amazon 等平台订单同步**。
...
1. **TikTok Shop OAuth + 订单同步**：实现真实 **`OrderSyncProvider`**（HTTP/SDK 仅限 Provider 包内；**订单核心模块不解析 TikTok 专有 JSON**）；密钥 **不入日志**。
2. **Shopee OAuth + 订单同步**：同上。
