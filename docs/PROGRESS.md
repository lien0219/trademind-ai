...
**最后更新**：2026-05-16（**AI 客服 MVP**：**`customer_conversations` / `customer_messages` / `customer_reply_suggestions`**；**默认 Prompt `customer_reply_generate`**（**scene `customer_service`**）；**客服 API**（**会话 CRUD、消息、mark-replied、AI 建议生成 / 编辑 / 采纳 / 废弃**）；**`ai_tasks.task_type=customer_reply_generate`** + **`conversation_id`**；**管理端** **`/customer/conversations`** 与 **`/customer/conversations/:id`** 工作台；**仅人工确认，不对外自动发送**；**COS/OSS 云存储**见上条历史记录。）
...
| **MVP 闭环** | 登录 → 配置 AI → 采集/草稿 → **AI 标题/描述** → **可选：客服工作台录入消息 → AI 生成建议 → 人工采纳（仅入库，不外发）**（依赖有效大模型） |
...
- **迁移**：启动时 `database.AutoMigrate` — `admin_users`、`settings`、**`operation_logs`**、**`files`**、**`products`、`product_images`、`product_skus`、`image_tasks`、`collect_batches`、`collect_tasks`、`collect_task_events`**、**`ai_prompts`、`ai_tasks`**、**`customer_conversations`、`customer_messages`、`customer_reply_suggestions`**；启动后 **`aiprompt.EnsureDefaults`** 写入默认 **`product_title_optimize`**、**`product_description_generate`**、**`customer_reply_generate`**（各仅在缺失时插入）；**`settings.EnsureImageDefaults`** 写入 **`image` 分组**默认键（**`provider=noop`**；**`removebg_*`**；**`openai_image_*`**；**`comfyui_base_url`（`http://127.0.0.1:8188`）**、**`comfyui_api_key`（加密，可选）**、**`comfyui_workflow_json`（json，空）**、**`comfyui_prompt_node_id` / `comfyui_image_node_id` / `comfyui_output_node_id`**、**`comfyui_timeout_sec`（180）**、**`comfyui_poll_interval_seconds`（2）**、**`comfyui_max_poll_seconds`（180）**；**`timeout_sec`**；**仅缺失键插入**）
...
- **操作日志**：`operation_logs` 表；模块 `internal/modules/operationlog`；**`GET /api/v1/operation-logs`**（分页；**action / username / resource / start / end（RFC3339）** 筛选）。写入场景：**登录成功/失败**、**logout**、**settings 批量保存成功/失败**、**test-ai / test-storage 成功/失败**（消息不落敏感配置明文）；**采集**：…；**商品**：…；**AI 标题**：…；**AI 描述**：…；**客服（MVP）**：**`customer.conversation.create` / `customer.conversation.update` / `customer.conversation.close`（含软删会话）**、**`customer.message.create`**、**`customer.conversation.replied`**、**`customer.reply_generate.success` / `customer.reply_generate.failed`**、**`customer.reply_suggestion.edit` / `accept` / `discard`**（**不落客户正文全量**；可含 **conversationId / suggestionId / platform / 长度摘要**）；**图片任务**：…
...
- **AI 客服（MVP，不外发）**：模块 **`internal/modules/customerchat`**；表 **`customer_conversations`**（**`platform` / 可空 `shop_id` / 可空 `external_conversation_id` / `customer_*` / `status` open·pending_reply·replied·closed / `last_message_at` / 软删**）、**`customer_messages`**（**`role` customer·agent·ai**；**`source` manual·imported·platform·ai_suggestion**；**JSONB `raw_data`**）、**`customer_reply_suggestions`**（关联 **`ai_tasks`**，**`suggested_reply` / `edited_reply` / `status` generated·edited·accepted·discarded**；**不含任何第三方 API Key**）。**API（均 JWT）**：**`GET/POST /api/v1/customer/conversations`**（筛选 **platform/status/customerName/start/end**）；**`GET/PUT/DELETE /api/v1/customer/conversations/:id`**；**`GET/POST …/:id/messages`**；**`POST …/:id/mark-replied`**（录入 **agent** 消息并 **replied**）；**`POST …/:id/ai/generate-reply`**（走 **AI Gateway**，写 **`ai_tasks`** + **`customer_reply_suggestions`**，**不自动创建外发消息**）；**`PUT /api/v1/customer/reply-suggestions/:id`**、**`POST …/accept`**、**`POST …/discard`**。管理端：**`/customer/conversations`**、**`/customer/conversations/:id`**（**`services/customer.ts`**）。
...
- **客服（AI MVP）**：路由 **`/customer/conversations`**（**ProTable** 会话列表 + 新建）与 **`/customer/conversations/:id`**（**消息时间线 + AI 建议区**：生成 / 编辑 / 采纳 / 废弃 / 复制；**不自动外发**）；**`services/customer.ts`**。
...
│       └── modules/         # auth、admin、settings、**operationlog**、**files**、**product**、**collect**、**aiprompt**、**aitask**、**imagetask**、**customerchat**
...
19. **AI 客服**：首版 **已落地**（§3.2 **手工会话 + AI 建议 + 人工采纳**）；**未接** TikTok/Shopee 等 **真实客服 API**、**不自动发送**、**无订单上下文关联**、**无 WebSocket**。
...
1. **订单/会话上下文**：客服侧 **关联订单号、物流、SKU**（需业务模型与安全脱敏）。
2. **店铺授权 + 平台客服 API**：TikTok / Shopee / Lazada / Amazon 消息 **拉取/发送**（在 **不破坏不外发默认** 的策略上扩展）。
