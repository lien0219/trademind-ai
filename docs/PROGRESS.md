...
**最后更新**：2026-05-16（**多平台开放平台应用 Schema 化**：各 **Platform Provider** 下发 **`AppConfigSchema()`**，预置 **`platform_tiktok`、`platform_shopee`、`platform_lazada`、`platform_amazon`、`platform_aliexpress`、`platform_shopify`、`platform_woocommerce`、`platform_ebay`、planned（**Temu / SHEIN / custom**）；**`GET /api/v1/platform/providers`** 携带 **`appConfigSchema`/`settingsGroupKey`**；**`GET`/`PUT /api/v1/platform/settings/:platform`** 校验字段、写入 **`settings`**、**AES-GCM**、**masked `****`** 不覆盖密钥、**`platform.settings.update`** 日志；**/settings/platforms** 动态表单 Tabs；**TikTok** 仍仅从 **`platform_tiktok` + shop tokens** 取数，缺必填项报错 **`platform config incomplete: please configure platform_tiktok…`**（`ErrIncompleteCred`）；**planned** OAuth/同步仍 **未接线** — §3/§7）
...
- **迁移**：启动时 `database.AutoMigrate` — `admin_users`、`settings`、**`operation_logs`**、**`files`**、**`products`、`product_images`、`product_skus`、`image_tasks`**、**`orders`、`order_items`、`order_shipments`、`order_sync_tasks`**、**`shops`、`shop_auth_tokens`**、`collect_batches`、`collect_tasks`、`collect_task_events`、**`collect_rules`**、**`ai_prompts`、`ai_tasks`**、**`customer_conversations`**（含可空 **`order_id`**、**`shop_id`**）、**`customer_messages`、`customer_reply_suggestions`**；启动后 **`aiprompt.EnsureDefaults`**（含 **`migrateCustomerReplyGenerateOrderContext`** 对存量 **`customer_reply_generate`** 占位符升级）、写入默认 **`product_title_optimize`**、**`product_description_generate`**、**`customer_reply_generate`**（各仅在缺失时插入）；**`settings.EnsureImageDefaults`** 写入 **`image` 分组**默认键（**`provider=noop`**；**`removebg_*`**；**`openai_image_*`**；**`comfyui_base_url`（`http://127.0.0.1:8188`）**、**`comfyui_api_key`（加密，可选）**、**`comfyui_workflow_json`（json，空）**、**`comfyui_prompt_node_id` / `comfyui_image_node_id` / `comfyui_output_node_id`**、**`comfyui_timeout_sec`（180）**、**`comfyui_poll_interval_seconds`（2）**、**`comfyui_max_poll_seconds`（180）**；**`timeout_sec`**；**仅缺失键插入**）
- **Redis**：`internal/rdb`（go-redis），连接失败 **仅告警**，服务继续（健康检查体现 `redis: skipped/degraded`）。
- **健康检查**：`GET /health`、`GET /api/v1/health`（含 DB/Redis 检查；**`data.collectQueue`**：`enabled`、`name`、`redisAvailable`、`depth`、`workerEnabled`、`workerConcurrency`；**`data.imageQueue`**：**`enabled`、`name`、`redisAvailable`、`depth`、`workerEnabled`、`workerRunning`、`concurrency`**；**`data.orderSyncQueue`**：**`enabled`、`name`、`redisAvailable`、`depth`、`workerEnabled`、`workerRunning`、`concurrency`**；**不对 Collector 做 HTTP 探测**以免拖慢健康接口）。队列开启且 Redis **Ping 正常但 LLEN 不可得**时整体 **`status` 可标记 `degraded`**。
...
- **操作日志**：`operation_logs` 表；模块 `internal/modules/operationlog`；**`GET /api/v1/operation-logs`**（分页；**action / username / resource / start / end（RFC3339）** 筛选）。写入场景：**登录成功/失败**、**logout**、**settings 批量保存成功/失败**、**platform.settings.update**（Partner 应用配置；**不落明文密钥**）、**test-ai / test-storage 成功/失败**（消息不落敏感配置明文）；**采集**：…；**商品**：…；**AI 标题**：…；**AI 描述**：…；**内部订单**：**`order.create` / `order.update` / `order.delete`（软删）**、**`order.item.{create|update|delete}`**、**`order.shipment.{create|update|delete}`**；**订单同步任务**：**`order.sync.create` / `order.sync.running` / `order.sync.success` / `order.sync.failed` / `order.sync.retry`**（**不落 token / 不全量 raw_data**）；**客服（MVP）**：**`customer.conversation.create` / `customer.conversation.update` / `customer.conversation.close`（含软删会话）**、**`customer.conversation.link_order`**、**`customer.message.create`**、**`customer.conversation.replied`**、**`customer.reply_generate.success` / `customer.reply_generate.failed`**、**`customer.reply_suggestion.edit` / `accept` / `discard`**（**不落客户正文全量**；可含 **conversationId / suggestionId / platform / 长度摘要**）；**图片任务**：…
...
- **统一店铺（平台授权基座，MVP）**：模块 **`internal/modules/shop`**；表 **`shops`**、**`shop_auth_tokens`**（敏感值 **AES-GCM**，API **脱敏 + masked 不覆盖**）。**`internal/providers/platform`**：**`Registry`** + **`manual` / `mock` / `planned`（各带 `AppConfigSchema` 预置）**；**TikTok Shop** 在 **`api/router`** 绑定 **`tiktok.BindShops` + `RegisterProvider()`**，状态 **`beta`**，能力 **`CapOrderSync`/`CapShopInfo`**。**应用级（部署/租户 settings）**：**`GET /api/v1/platform/providers`** 返回 **`appConfigSchema` + `settingsGroupKey`**；**`GET`/`PUT /api/v1/platform/settings/:platform`** 按 schema 写入 **`settings` 分组**（如 **`platform_tiktok`**、**`platform_shopee`** …），**敏感项加密**，**`platform.settings.update`** 审计；**不**把 **access/refresh token** 写入 settings。**店铺级授权**：**token / shop_cipher** 等 → **`shop_auth_tokens`**；Drawer **可选**覆盖 **App Key/Secret/redirectUri**（合并进 **`TestConnectionRequest`**）。**OAuth**：**authorize-url** / **callback**（Redis **state**），**callback 仅写 token**。**`tiktok.ResolveRuntime`** 合并 **settings `platform_tiktok`（小写键）+ 店铺覆盖**，**不写死域名**；缺 Partner 必填项返回 **`platform config incomplete: please configure platform_tiktok.app_key, app_secret and redirect_uri`**（**`ErrIncompleteCred` 常量别名**）。**`shop.*` 日志不写密钥**。与订单/客服：**`shopSummary`**。
- **平台订单同步（框架）**：模块 **`internal/modules/ordersync`**；表 **`order_sync_tasks`**（UUID、`shop_id`、`platform`、`task_type=order_sync`、状态、**JSONB `input`/`output`**，**不落 token**）；**Redis `ORDER_SYNC_QUEUE_*`** + **Worker**（**`ORDER_SYNC_QUEUE_ENABLED=false`** 时同步执行）；**编排**：校验 **`CapOrderSync`**／店铺 **`active`**；**`tiktok`（beta）** 走 **`SyncOrders`** 真实拉单；**`planned`/`manual`** 仍为 **501／不支持** 语义；**`mock`** 模拟。**API**：**`POST /api/v1/shops/:id/sync-orders`**、**`GET /api/v1/order-sync/tasks`**（筛选 **shopId/platform/status/start/end**）、**`GET …/tasks/:id`**、**`POST …/tasks/:id/retry`**。**`order.UpsertSyncedOrders`** 入库 **`orders`/`order_items`/`order_shipments`**；**`/health`** **`orderSyncQueue`**。**管理端**：**`/orders/sync-tasks`**、**`/shops` 同步**；**`/orders`** 列表 **platform/`externalOrderId`**。前端不直连平台 API。
...
- **AI 客服（MVP，不外发）**：模块 **`internal/modules/customerchat`**；注入 **`Orders *order.Service`**、**`Shops *shop.Service`**（**`shopSummary` 批量摘要**）；表 **`customer_conversations`**（同上 + 可空 **`order_id`**、可空 **`shop_id` → `shops.id`**）、**`customer_messages`**、**`customer_reply_suggestions`**（不变）。**`GET …/conversations/:id`** 带出 **`orderId`/`orderSummary`**、**`shopId`/`shopSummary`**。**PUT …/conversations/:id**：**`UpdateConversationBody`** 可选 **`orderId`**（UUID 绑定；**空字符串解除关联**）、**`shopId`**（同上）；**不存在订单/店铺**报错。**`POST …/:id/ai/generate-reply`**：若已关联订单，拼装 **订单/明细/发货 JSON** …（不变）。**其余 API**：同前。**管理端**：`/customer/conversations`、`/customer/conversations/:id`，**工作台** **选店铺 Modal**、选单 Modal、**不自动外发**。**`services/customer.ts`**：**`shopId`/`shopSummary`**。
...
- **Settings 与各分组页面**：`GET/PUT /api/v1/settings`，**`group_key`**：**`system`、`ai`、`storage`、`collector`、`security`、`image`** 与历史 **`platform_tiktok` 仍可经通用 settings 读写**（推荐优先走 **`/platform/settings/:platform`**）；敏感项 **脱敏**。**`/settings/platforms`（菜单：平台开放配置）**：数据来源 **`GET /api/v1/platform/providers`** Tabs + **`provider.appConfigSchema` 动态表单** → **`PUT /api/v1/platform/settings/:platform`**；仍可辅以 **`POST /api/v1/settings/test-platform-tiktok`** **仅 TikTok 结构自检**。**`image` 分组**详见 §3.2。**存储页**：**`kind` 切换**（**本地** `public_base`/`local_root`；**`s3`/`r2`/`minio`**：**`s3_*`** — Endpoint/Region/Bucket、密钥 **密码输入 + 脱敏 + masked 不覆盖**、Path-style、`s3_use_ssl`、可选 **`s3_presign_*`、`s3_public_base`**；**COS**：**`cos_bucket`/`cos_region`/`cos_secret_*`**（可选 **`cos_app_id`/`cos_endpoint`/`cos_public_base`/`cos_use_https`**）；**OSS**：**`oss_endpoint`/`oss_bucket`/`oss_access_key_*`**（可选 **`oss_public_base`/`oss_use_https`**）— **前端不经由浏览器直连 COS/OSS**。**测试**：`test-ai` / **`test-storage`（local / S3-compat HeadBucket / COS HeadBucket / OSS List(max1)）**；**上传测试** **`POST /api/v1/files/upload`**（亦 **不经由浏览器直连第三方 AI/Image**）。
...
- **店铺管理（授权基座）**：路由 **`/shops`**；**`services/shops.ts`** + **`platformOpen.ts`**：**TikTok OAuth**；默认 **不写每店 App Key/Secret**，引导 **「设置 → 平台开放配置」**；**`formatTiktokPlatformErr`** 映射 **`platform config incomplete`** 等文案；**同步订单 Modal**、`/orders/sync-tasks`。**`SHOP_*` / `PLATFORM_PROVIDER_STATUS`**。
- **客服（AI MVP）**：路由 **`/customer/conversations`**（**ProTable** 会话列表 + 新建，可选 **`shopId`**）与 **`/customer/conversations/:id`**（**关联店铺** 卡片【**选择店铺 Modal** / 解绑】、**关联订单** 卡片【选单 Modal / 解绑】、**订单与物流概要**；**消息时间线 + AI 建议区**：**风险提示 Alert**、生成 / 编辑 / 采纳 / 废弃 / 复制；**不自动外发**）；**`services/customer.ts`**。
...
│       ├── providers/       # **`storage`**（local / s3 / cos / oss 等）、**`ai`、`image`、`platform`**（含 **`tiktok` beta，`OrderSyncProvider`**）
...
19. **AI 客服与内部订单上下文**：工作台已支持 **绑定内部订单**（含 **同步入库的 `orders`**）；**仍无** 各平台 **实时客服消息收发** / **WebSocket**。**TikTok Shop（beta）订单** 可走 **`SyncOrders`** 入库；**客服外发与消息拉取** 未接。
...
25. **TikTok Shop 仍为 beta**：已具备 **Schema 化 `platform_tiktok` 配置页 + OAuth + `SyncOrders`**；生产仍需关注 OpenAPI 演进、分页与字段完整性。**Shopee / Lazada / Amazon** 等 **`planned`**：后台 **可先保存 `platform_*` 应用参数**，**TestConnection / OAuth / 订单同步** 仍 **未实现**（返回 **ErrNotImplemented / 501** 语义）。**平台级「测试连接」聚合 API**、**按店铺覆盖应用密钥** 等后续可增强。
26. **未实现真实 Shopee 订单同步**（需 OAuth + 开放平台订单 API）。
27. **未实现 Lazada / Amazon 等平台订单同步**。
28. **未实现平台客服消息拉取 / 发送**（与订单同步管道独立）。
29. **未实现商品刊登 / 库存同步**。
30. **多实例 Worker heartbeat / 任务租约**：订单同步 Worker 仍为进程内 **`workerRunning` 标记**（与采集、图片 Worker 同类遗留）。

---

## 8. 下一步开发计划（建议顺序）

1. **TikTok Shop 深化**：生产实测分页与字段；按需补 **`docs/platform-provider.md` / `api.md`**。**平台 Partner 配置的「一键测试」（非 TikTok）**可按 schema 演进。
2. **Shopee OAuth + 订单同步**：真实 **`OrderSyncProvider`**（HTTP/SDK **仅在 Provider**；密钥 **不落日志**）。
3. **平台客服消息能力**：消息 **拉取** 与 **人工确认后发送**（延续 **不外发默认**；与 **`order_sync_tasks`** 管道分离）。
4. **多实例 Worker heartbeat / 租约**：采集 / 图片 / **订单同步** Worker（§7）。
5. **Collector 演进（补强 / 扩展）**：**AliExpress**：**批量链路开放前**，补强 **SKU 稳定性与异步详情**。**新源**：在 **`pdd` / `taobao` / `shein_temu`** 中选 **其一**落地 **真实结构化解析**（**对齐 `NormalizedProduct`**）。**自定义**：**规则可视化编辑器**、**批量开放（若需）**、**SSRF 深度防护**。反爬：**SKU 多维、异步详情 iframe、人机分流**（**仅限 Collector**，Go **不写 PDP 解析**）。
6. **订单同步深化**：租户隔离、**`external_order_id`/`order_no` 冲突策略**、SKU 与 **`products` 草稿联动**、生产平台分页 **`cursor`** 持久化策略。
...
6. **后端**：Handler 薄、Service 编排、**外部调用带超时**；**采集 / 图片 / 订单同步任务**均已接 **Redis 列表 + 进程内 Worker**（**`ORDER_SYNC_QUEUE_ENABLED=false`** 时订单同步可在请求内同步执行）（**可观测指标/告警**仍可加强）。
