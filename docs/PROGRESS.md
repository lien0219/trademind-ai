...
**最后更新**：2026-05-16（**OpenAI Image Provider**：**`settings.image.openai_image_*`**（**`openai_image_api_key` 加密**、`openai_image_base_url` 空则 **`https://api.openai.com/v1`**、默认模型 **`gpt-image-1`**、`size`/`quality`/`background`）；**不要求**回退 **`settings.ai.api_key`**（代码注释说明）；**`internal/providers/image/openaiimage` HTTP Client** + **`factory` `openaiImageProvider`**；**`generate_scene`**：服务端 **`assembled_prompt`** 组装（可读 **`productId`** 补强标题/`ai_title`/描述/`raw_data` 属性摘要/类目 hint），**Worker** 照常消费 **`image:tasks`**，**`/images/generations`** **Context+http.Client Timeout**，**errors 记入 `error_message`**；**PNG 字节** **`files.SaveProcessed`**（**`openai-image-<taskId>`**、`image-tasks/YYYYMMDD/`）；**output** **`resultUrl`/`resultFileId`/`model`/`contentType`**；**`remove_background`** **强制 `provider=removebg`**（入库与执行双保险）；**`replace_background`** 对 **`openai_image`** **仍标记未实现**；**可选无源图** 仅限 **`generate_scene` + OpenAI Image**。**管理端**：**`/settings/image`** 表单；**`/ai/image-tasks`** 创建 **generate_scene** + **预览**；**商品详情图片 Tab**：**generate_scene** + **prompt**。**遗留**：ComfyUI、**OpenAI replace_background（真实图像编辑链路）**、**本地非公网图 multipart→remove.bg**、**图片任务自动退避重试**，见 §7/§8。）
...
| 维度 | 状态 |
|------|------|
| **路线图阶段** | **第 5 阶段（采集）**保持；**第 6 阶段（AI 图片）**：**remove.bg 去背景** + **OpenAI Image `generate_scene`** 已与 **`image_tasks` + Storage + `files` + Worker** **贯通**；**`openai_image` `replace_background` 仍为占位错误**（见 §7）；其余同前 |
...
- **迁移**：启动时 `database.AutoMigrate` — `admin_users`、`settings`、**`operation_logs`**、**`files`**、**`products`、`product_images`、`product_skus`、`image_tasks`、`collect_batches`、`collect_tasks`、`collect_task_events`**、**`ai_prompts`、`ai_tasks`**；启动后 **`aiprompt.EnsureDefaults`** 写入默认 **`product_title_optimize`**、**`product_description_generate`**（各仅在缺失时插入）；**`settings.EnsureImageDefaults`** 写入 **`image` 分组**默认键（**`provider=noop`**；**`removebg_*`**；**`openai_image_base_url` / `openai_image_api_key`（加密）/ `openai_image_model`（默认 `gpt-image-1`）/ `openai_image_size` / `openai_image_quality` / `openai_image_background`**；**`comfyui_*`**；**`timeout_sec`**；**仅缺失键插入**）
...
- **AI 图片任务**：模块 **`internal/modules/imagetask`**；表 **`image_tasks`**（**`task_type`**：`remove_background` / …；**`status`**：**`pending` / `running` / `success` / `failed` / `cancelled`**；**JSONB `input` / `output`**；**`source_image_id`** → **`files` / `product_images`** 解析 **`public_url`**；**`result_file_id` / `result_url`**；**不落库图片二进制**）。**Image Provider**：**`internal/providers/image`**：**`noop`**；**`removebg`** 子 Client（**`RemoveBackgroundPNG`**）；**`openaiimage`** 子包（HTTP **`POST /images/generations`**）；**`factory.NewForTask`：`noop` | `removebg` | `openai_image`**，读 **`settings.image`**（密钥 **明文解密不写日志**，**首期不回退 `settings.ai.api_key`**，见工厂注释）。**`remove_background`**：**入库与执行锁定 `provider=removebg`**；仅 **公网 `image_url`**；PNG → **`files.SaveProcessed`**（**`processedObjectTag` = `removebg`**）；**output** **`resultUrl`/`resultFileId`/`provider`/`contentType`**。**`generate_scene` + `openai_image`**：**允许无 source（纯文案构图），也可附上参考图 **`source`**；**`imagetask.prepareGenerateSceneHints` → `assembled_prompt` → `/images/generations`**；响应 **`b64_json` 或可下载 `url`** → **`SaveProcessed`**（tag **`openai-image`**）；成功 **`output`** 含 **`model`/`contentType`**；操作日志 **`image.task.success`** 可带 **`model`（明文模型名）**。**`replace_background`**：**openai_image 仍为明确「未实现」错误**。**异步队列**：**`IMAGE_QUEUE_ENABLED`**（默认 **true**）、**`IMAGE_QUEUE_NAME`**（默认 **`image:tasks`**）、**`IMAGE_WORKER_CONCURRENCY`**、**`IMAGE_TASK_TIMEOUT_SECONDS`**（与 **`settings.image.timeout_sec`** 取 **更小** 作为 **context 上限**）；**`IMAGE_AUTO_RETRY_*`** 环境变量 **已加载占位**，**自动退避重试 scheduler 未实现**（**人工 retry**）。**`POST /api/v1/image/tasks`**：除 **`generate_scene`** 且选定/默认 **`openai_image`** 可无源图外，仍需 **`sourceImageId`/`sourceImageUrl`**；**`CreateAndPersist` → `pending`**；队列开启时 **Redis `LPUSH`**（消息仅 **`taskId` / `taskType` / `provider` / `createdBy` / `requestId`**）；**立即返回**（**不在 HTTP 内调 remove.bg**）；**Redis 不可用**：**503** **`Image queue unavailable`**，**已创建行则回滚删除**；**队列关闭**：**同请求同步**跑 **`ProcessSyncAfterCreate`**（开发模式）。**Worker**：**`BRPOP`** → **`ProcessQueuedTask`**：**CAS `pending` → `running`** → 既有 **dispatch / SaveProcessed**；**`operationlog.WriteBackground`** **`image.task.success` / `image.task.failed`**（**`CreatedBy` 归因**）。**API 列表/详情**：**`GET /image/tasks`、`:id`**；**`GET /image/tasks/monitor`**（队列深度、Worker、**按 status 聚合计数**）；**`POST .../:id/retry`**：**仅 `failed`** → **`pending`** 清空 **结果字段** + **再入队**（或同步模式立即执行）；失败时 **`image.task.retry`**。HTTP 创建写 **`image.task.create`**。管理端 **`/settings/image`**、**`/ai/image-tasks`**（轮询、**处理中** 态、复制 URL / 可选加入商品图）、商品详情 **图片 Tab**。**其它任务类型**：noop 仍为 **`resize`/`enhance` 回显**；removebg **`remove_background` only**；openai **`generate_scene` only**。
...
- **系统 / AI / 存储 / 采集服务 / 安全 / 图片 AI** 设置页：`GET/PUT /api/v1/settings`，按 **groupKey**（`system`、`ai`、`storage`、`collector`、`security`、**`image`**）读写 **snake_case** **`itemKey`**；**`image` 分组**现已含 **`provider`（noop/removebg/openai_image）**与 **`removebg_*`** / **`openai_image_base_url`、`openai_image_api_key`（加密）、`openai_image_model`、`openai_image_size`、`openai_image_quality`、`openai_image_background`、`timeout_sec`** 等敏感项同上 **脱敏 + masked PUT 语义**；**存储页**：**`kind` 切换**（**本地** `public_base`/`local_root`；**`s3`/`r2`/`minio`**：**`s3_*`** — Endpoint/Region/Bucket、密钥 **密码输入 + 脱敏 + masked 不覆盖**、Path-style、`s3_use_ssl`、可选 **`s3_presign_*`、`s3_public_base`**）；**COS/OSS** **占位标签 + Alert**。**测试** `test-ai` / **`test-storage`（云端 HeadBucket）**；**上传测试** **`POST /api/v1/files/upload`**（前端 **不直连** S3/R2/MinIO）。
...
- **商品草稿**：路由 **`/product/drafts`**，`ProTable` → **`GET /api/v1/products`**；**`/product/drafts/:id`** **Tabs**（基础、AI 标题/描述、**图片管理**（上传、`createProductImage`、**AI 图片任务**：**resize/noop**、**remove_background/remove.bg**、**generate_scene/openai_image + Prompt**，异步提示 + **`/ai/image-tasks`**、reorder）、**SKU 表**、最近 AI 任务）；**`/ai/prompts`**、**`/ai/tasks`**、**`/ai/image-tasks`**（~4s 轮询）。**`products.ts` / `imageTasks.ts`** 封装 API。
...
- [x] **AI 图片任务**：**remove.bg** + **OpenAI Image `generate_scene`** + **Redis Worker**（详见 §3.2）；**`openai_image` `replace_background` 仍为占位错误**。**未完成**：ComfyUI、**图片自动重试 scheduler**、**本地非公网 multipart→remove.bg**、**图像编辑（替换背景 OpenAI）**
...
| AI 图片 | **`internal/providers/image`**：**`noop`** + **`removebg` Client** + **`openaiimage` Client（**generations**）；**factory**：**`noop` | `removebg` | `openai_image`**；**异步队列**同 §3.2（**单进程 Worker**）；**OpenAI Image 密钥与 `settings.ai.api_key` 分离（不回退）**；**未完**：ComfyUI、**真实 `replace_background`（图像编辑）**、multipart remove.bg、scheduler 自动重试 |
...
17. **AI 图片（剩余）**：**OpenAI Image `generate_scene` 已与 Storage + Worker 贯通**，但 **`provider=openai_image` `replace_background` 仍为「未实现」**；仍需 **ComfyUI**；**本地非公网源图 multipart 上传 remove.bg**；**`IMAGE_AUTO_RETRY_*` scheduler** 仍为占位。
18. **AI 客服**：**未完成**。
...
1. **图片 Provider（剩余）**：**ComfyUI**（读 **`settings.image`**）；Flux / 即梦 预留；补足 **OpenAI `replace_background`（multipart 图像编辑链路）** 若产品需要。
2. **图片任务增强**：自动退避重试（落实 **`IMAGE_AUTO_RETRY_*`**）；`/image/tasks/monitor` **告警**可做；本地非公网图 **multipart** 直传 remove.bg。
3. **COS / OSS 独立 Provider**：在 **腾讯云 / 阿里云** 原生 SDK（或专有域名）上与 **`files`/设置页**对齐，而不走 S3 兼容端点。
4. **多实例 Worker**：**Redis heartbeat** 或注册表（采集 / 图片 Worker）。
