...
**最后更新**：2026-05-17（**平台客服消息同步框架（beta/MVP）**：**`CustomerMessageProvider`**（**`PullMessages` / `SendMessage`**）；**`internal/modules/customersync`**（**`customer_message_sync_tasks`** + **`CUSTOMER_MESSAGE_SYNC_*` 队列** + Worker **`customer_message_sync`**）；**复用 `settings.platform_*` + `shop_auth_tokens`**；**mock** 可拉取/发送模拟消息；**TikTok / Shopee / Lazada / Amazon** 客服 API **占位未实现**；**`POST /shops/:id/sync-customer-messages`**、**`GET /customer/message-sync/tasks*`**、**`POST /customer/conversations/:id/send-platform-message`**（**人工点击**、**不外发**）；**`/customer/message-sync-tasks`**、会话列表/工作台 **拉取平台消息**；**Reaper / `/health` workers / `/workers/monitor`** 含 **`customer_message_sync`**）
...
- **平台客服消息同步（框架）**：模块 **`internal/modules/customersync`**；表 **`customer_message_sync_tasks`**（租约 + **`input`/`output` JSONB**，**不落 token**）；**`internal/providers/platform.CustomerMessageProvider`**（**`PullMessages`/`SendMessage`**；**mock** 可拉取/发送仿真消息；**tiktok/shopee/lazada/amazon** 骨架返回 **`platform customer message provider not implemented`**；**manual** 不支持）；**`customerchat.SyncPlatformCustomerMessages`** 幂等 upsert **`customer_conversations`/`customer_messages`**（裁剪 **`raw_data`**）；**`POST /api/v1/shops/:id/sync-customer-messages`**、**`GET /api/v1/customer/message-sync/tasks`**、**`GET .../:id`**、**`POST .../:id/retry`**；Redis **`CUSTOMER_MESSAGE_SYNC_*`** + Worker **`customer_message_sync`**（队列关闭时可同步执行）；**`/health` `customerMessageSyncQueue`**；**TaskReaper** 回收 **`customer_message_sync_tasks`** 超时 **`running`**；**`POST /api/v1/customer/conversations/:id/send-platform-message`**：**仅人工**触发外发，成功后 **`source=platform`** 写 **`customer_messages`**，**`accept` 建议仍仅内部记录**。
...
19. **AI 客服与内部订单上下文**：工作台已支持 **绑定内部订单**（含 **同步入库的 `orders`**）。**已落地** **平台客服消息同步框架**（**`customersync`**、**人工外发**、**mock** 全链路可测）；**TikTok / Shopee / Lazada / Amazon** 的 **真实 `PullMessages`/`SendMessage` 仍占位未实现**。仍无各平台 **实时消息 WebSocket 推送**。
...
33. **TikTok / Shopee / Lazada / Amazon 真实客服消息 API**：`PullMessages` / `SendMessage` **未接入**（当前为 **骨架返回未实现**；**订单同步不受影响**）。
34. **平台商品刊登未完成（多平台归纳）**。

## 8. 下一步开发计划（建议顺序）

1. **TikTok / Shopee / Lazada / Amazon 真实客服消息 API 接入**（在各自 **`providers/platform/<name>/`** 内实现 **`PullMessages`/`SendMessage`**，不侵入 `customerchat` 核心）。
2. **平台商品刊登**（多平台，与订单管道解耦）。
3. **Amazon SP-API 生产实测与限流优化**（凭证、`429`、多区域 endpoint、字段完整性）。
4. **Collector 反爬与稳定性增强**。
5. **TikTok / Shopee / Lazada 订单同步生产实测**：分页、字段、沙箱与 **`cursor`/offset**；按需补 **`docs/platform-provider.md` / `api.md`**。
