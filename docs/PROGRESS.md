...
**最后更新**：2026-05-17（**TikTok Shop 真实库存同步 API（beta）**：**`tiktok.SyncInventory`** 走 **`POST /product/{api_version}/products/{id}/inventory/update`**；**`/api/v1/platform/providers`** **`tiktok.inventory_sync`**=`beta`（**mock** **`available`** 不变）；复用 **`settings.platform_tiktok`**、**`settings.platform_publish_tiktok`**（默认 **`warehouse_id`**；可由 **`inventory_sync_tasks.input.options.warehouse_id`** 覆盖）、**`shop_auth_tokens`**、**`product_publications.external_product_id` / `product_publication_skus.external_sku_id`**；成功后更新 **`product_publication_skus.stock`** + **`inventory_change_logs`**（**Worker 既有路径**）；**`ErrPlatformInventorySyncPermissionDenied`**；**`shop.TikTokShopsBridge.TikTokPublishSettings`**。）**上一轮（库存框架 + 刊登 beta，仍适用）**：**库存同步**：**`inventory_change_logs`/`inventory_sync_tasks`**、**Redis `INVENTORY_SYNC_*`**、Worker **`inventory_sync`**……**TikTok / Shopee / Lazada 商品刊登真实 API（beta）**……Amazon **`product_publish`**=`planned`。）
...
- **库存同步基础框架 + TikTok 真实写入（beta）**：模块 **`internal/modules/inventory`**；迁移 **`inventory_change_logs`**、**`inventory_sync_tasks`**。**`InventorySyncProvider.SyncInventory`**：**mock** 模拟成功 · **manual** 显式不支持 · **TikTok** **`inventory_sync`**=`beta`（**`providers/platform/tiktok/inventory_sync*.go`**，**复用 `signedPOSTJSONStatus` / `ensureFreshAccess` / `ResolveRuntime`**；**`warehouse_id`** 优先 **`tasks.input.options`**，否则 **`settings.platform_publish_tiktok`**；缺 **`warehouse_id`** → **`platform inventory config incomplete: missing warehouse_id`**；权限类错误 → **`platform inventory sync permission denied or not configured`**（**`ErrPlatformInventorySyncPermissionDenied`**））· **Shopee / Lazada / Amazon** **`inventory_sync`** 仍为 **`planned`**（**`ErrInventorySyncNotImplemented`**）。**`GET /api/v1/platform/providers`** 含 **`capabilityStatus.inventory_sync`**。**队列：** **Redis** **`INVENTORY_SYNC_*`**；Worker **`inventory_sync`**；**`/health`** **`inventorySyncQueue`**；**`/workers/monitor`** **`leasedTasks.inventorySync`**；**TaskReaper** 回收 **`inventory_sync_tasks`**。**JWT**：**`POST /api/v1/products/:id/skus/:skuId/adjust-stock`**（**`inventory.stock.adjust`**；默认更新 **`product_skus`**，可选 **`sync=true`** 建批量任务）；**`POST /api/v1/product-publication-skus/:id/sync-inventory`**、**`POST /api/v1/products/:id/sync-inventory`**；**`GET /api/v1/inventory-sync/tasks`**（含 **retry**）；**`GET /api/v1/products/:id/publication-skus`**；**`GET /api/v1/products/:id/skus/:skuId/inventory-logs`**、**`GET /api/v1/inventory/logs`**；**`inventory.sync.*`** 操作日志（**不写 token**）。成功回填 **`product_publication_skus.stock`** 并记 **`inventory_change_logs`**（含 **`sync_success`** 等）。管理端：**`/inventory/sync-tasks`**、草稿 **「库存」Tab**（**TikTok beta 指引**）、**`admin/src/services/inventory.ts`**；**.env.example** 含 **`INVENTORY_SYNC_*`**。
...
27. **Shopee / Lazada / Amazon 真实库存写入 API**：库存同步框架已就位 · **TikTok** 已实现 **`providers/platform/tiktok` `SyncInventory`（beta）**；其余三平台仍为 **`planned`**。**订单自动扣减 / 回补 **`product_skus`**（**`inventory_change_logs`** 预留 **`order_deduct`/`order_cancel_restore`**）**未实现**。
...
31. **TikTok 库存同步（beta）边界**：**`POST …/products/{product_id}/inventory/update`** 请求体与 **Partner 文档**需随版本校准；**多仓 / `location_id`** 等扩展未强制；**429 / 5xx / 超时** 错误文案带 **`retryable`** 提示由运营 **人工 retry**。
32. **Lazada 商品刊登（beta）边界**：依赖 **`settings.platform_publish_lazada`**（**`default_category_id` / 包裹重量·长宽高 / `delivery_option` / `default_brand_id`（写入 `brand`）** 等）与 **`attrs` + `options.lazada_attributes`** 填报类目必填属性；**`/image/migrate`**（公网图）+ **`/image/upload`**（Storage **Get**）；**`payload` JSON** **`Request.Product`** 随 **Open Platform** 演进需校准；**`publish_as_draft`** 仅落摘要，**不强改平台字段**。
...
1. **Shopee 真实库存写入 API**：在 **`providers/platform/shopee`** 实现 **`SyncInventory`**（**`inventory_sync` rollout**）。
2. **Lazada 真实库存写入 API**：在 **`providers/platform/lazada`** 实现 **`SyncInventory`**。
3. **Amazon 商品刊登真实 API 接入**（SP-API Listings / Feeds）；随后 **Amazon **`SyncInventory`****。
...
8. （可选后续）**订单驱动本地库存**：在 **`inventory_change_logs`** 中落地 **`order_deduct`/`order_cancel_restore`** 并联动 **`product_skus.stock`**。
9. **TikTok 库存同步生产实测**：**`inventory/update`** 与 **Partner 文档**对齐、**多仓**、**部分失败**摘要。
