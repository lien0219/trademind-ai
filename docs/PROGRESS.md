...
**最后更新**：2026-05-17（**Lazada 真实客服消息 API（beta）**：**`providers/platform/lazada`** 实现 **`CustomerMessageProvider.PullMessages` / `SendMessage`**（IM **`/im/session/list`** → **`/im/message/list`** → **`/im/message/send`**，路径集中于 **`paths.go`**；**复用 `ResolveRuntime`**、**`ensureFreshAccess`**、**`buildSignedQuery` / `signedGET` / `signedPOSTForm`**（与订单 **`client`** 同源签名）；**`settings.platform_lazada` + `shop_auth_tokens`**；**不写死 scope**；权限类 **`platform customer message permission denied or not configured`**；**429 / 5xx / 超时** 标记 **`retryable`**；会话分页 **`lz-im-v1:` cursor**；**`CustomerMessageImplementationStatus(lazada)=beta`**；**`GET /platform/providers`**：**TikTok / Shopee / Lazada **`customer_message`** = `beta`**；**Amazon** 仍为 **`planned`**；**mock / manual / TikTok·Shopee 订单与客服链路不受影响**；管理端 **`/shops`** 权限提示含 **Lazada**）
...
33. **Amazon 真实客服消息 API**：`PullMessages` / `SendMessage` **未接入**（**骨架返回未实现**；**订单同步不受影响**）。**TikTok / Shopee / Lazada** 已接 **`beta`**（见 §3.2 客服同步条）。
34. **平台商品刊登未完成**（多平台）。
35. **库存同步未完成**（多平台）。

## 8. 下一步开发计划（建议顺序）

1. **Amazon 真实客服消息 API 接入**（**`providers/platform/amazon`** 内实现 **`PullMessages`/`SendMessage`**；**TikTok / Shopee / Lazada** 已完成 **beta**）。
