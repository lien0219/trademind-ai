...
**最后更新**：2026-05-16（**自定义链接采集器（Collector `beta`）**：表 **`collect_rules`** + **`collect_tasks.request_options`**；**JWT API** **`GET/POST/PUT/DELETE …/collect/rules`**、**`…/enable|disable|test`**（测试走 Collector **`POST /v1/collect`** + **`options.rule`**，不落任务/草稿）；**Collector `sourceCustom`**（CSS Selector + **JSON-LD / OpenGraph / Meta** fallback，**NormalizedProduct** / **`raw.stateDigest`**）；**`POST /collect/tasks`** 支持 **`source=custom`** + **`ruleId`** 或域名自动匹配启用规则；Worker **快照 rule** 下发 Collector（**不写 PDP 解析进 Go**）；管理端 **`/collect/rules`**、**`/collect/tasks`** custom **规则下拉**；Hub **custom 批量**提示「自定义采集器暂不支持批量」。其它：**AliExpress beta**、1688 **available**；**拼多多 / 淘宝 / SHEIN·Temu** 仍为 **`planned`**。）
...
- **迁移**：启动时 `database.AutoMigrate` — `admin_users`、`settings`、**`operation_logs`**、**`files`**、**`products`、`product_images`、`product_skus`、`image_tasks`**、**`orders`、`order_items`、`order_shipments`**、`collect_batches`、`collect_tasks`、`collect_task_events`、**`collect_rules`**、**`ai_prompts`、`ai_tasks`**、**`customer_conversations`**（含可空 **`order_id`**）、**`customer_messages`、`customer_reply_suggestions`**；启动后 **`aiprompt.EnsureDefaults`**（含 **`migrateCustomerReplyGenerateOrderContext`** 对存量 **`customer_reply_generate`** 占位符升级）、写入默认 **`product_title_optimize`**、**`product_description_generate`**、**`customer_reply_generate`**（各仅在缺失时插入）；**`settings.EnsureImageDefaults`** 写入 **`image` 分组**默认键（**`provider=noop`**；**`removebg_*`**；**`openai_image_*`**；**`comfyui_base_url`（`http://127.0.0.1:8188`）**、**`comfyui_api_key`（加密，可选）**、**`comfyui_workflow_json`（json，空）**、**`comfyui_prompt_node_id` / `comfyui_image_node_id` / `comfyui_output_node_id`**、**`comfyui_timeout_sec`（180）**、**`comfyui_poll_interval_seconds`（2）**、**`comfyui_max_poll_seconds`（180）**；**`timeout_sec`**；**仅缺失键插入**）
