...
**最后更新**：2026-05-17（**TikTok 真实客服消息 API（beta）**：**`providers/platform/tiktok`** 实现 **`CustomerMessageProvider.PullMessages` / `SendMessage`**（**`customer_service/{api_version}/…`** 路径常量 + 签名为 **`signedPOSTJSONStatus`**；**不写死 scope**；权限类错误 → **`platform customer message permission denied or not configured`**；**复用 `settings.platform_tiktok` + `shop_auth_tokens` + token 刷新 + `shop_cipher`**；**`customersync` / Worker 不变**；入库 **`customer_conversations` / `customer_messages`** 仍由 **`customerchat.SyncPlatformCustomerMessages`**；**`GET /api/v1/platform/providers`**：**TikTok **`customer_message`** = `beta`**；**Shopee / Lazada / Amazon** 仍为 **`planned`**；**mock / manual** 不变；**管理端 `/shops`** 拉取客服消息放行 **`beta`**；**发送平台消息** API **403** 返回中文权限提示；**订单同步不受影响**）
...
19. **AI 客服与内部订单上下文**：工作台已支持 **绑定内部订单**（含 **同步入库的 `orders`**）。**已落地** **平台客服消息同步**（**`customersync`**、**人工外发**、**mock** 全链路可测）；**TikTok** **`PullMessages`/`SendMessage` 已真实接入（beta**，依赖 Partner **Customer Service** 路径与 **`api_version`**；字段映射需随文档校准）；**Shopee / Lazada / Amazon** 仍为占位。**`GET /platform/providers`**：**TikTok `customer_message`** 为 **`beta`**。仍无各平台 **实时消息 WebSocket 推送**。
...
33. **Shopee / Lazada / Amazon 真实客服消息 API**：`PullMessages` / `SendMessage` **未接入**（当前为 **骨架返回未实现**；**订单同步不受影响**）。**TikTok** 已接 **beta**（见 §3.2 客服同步条）。
34. **平台商品刊登未完成（多平台归纳）**。

## 8. 下一步开发计划（建议顺序）

1. **Shopee / Lazada / Amazon 真实客服消息 API 接入**（在各自 **`providers/platform/<name>/`** 内实现 **`PullMessages`/`SendMessage`**，不侵入 `customerchat` 核心；**TikTok** 主线已完成 **beta**）。
