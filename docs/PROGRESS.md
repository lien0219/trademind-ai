...
**最后更新**：2026-05-17（**库存同步基础框架（本轮）**：**`inventory_change_logs`/`inventory_sync_tasks`**、**`platform.InventorySyncProvider`**、**mock** `available` **`SyncInventory`**、**`tiktok`/`shopee`/`lazada`/`amazon`** **`inventory_sync`**=`planned`（**不提供真实写入**）、Redis **`INVENTORY_SYNC_*`**、Worker **`inventory_sync`**、**`/health` `inventorySyncQueue`**、路由 **`adjust-stock`/公开映射 `publication-skus`/任务列表与重试**、前端 **`/inventory/sync-tasks`** 与草稿 **「库存」Tab**。）**上一轮（刊登 beta，仍适用）**：**TikTok / Shopee / Lazada 商品刊登真实 API（beta）**：**`tiktok`** · **`PublishProduct`**；**`shopee`** · **`PublishProduct`**（**`upload_image` / `add_item` / `init_tier_variation`**）；**`lazada`** · **`PublishProduct`**（**`signedPOSTForm` `/product/create`** + **`/image/migrate`** 与 **`/image/upload` 多图**，**`payload` JSON** **`Request.Product`**）；均复用 **`settings.platform_*`**、**`settings.platform_publish_*`**、**`shop_auth_tokens`**、**`product_publish_tasks`** Worker；**`platformlazada.BindPublishImages`** 与 TikTok/Shopee 共用 **`tikTokListingImageFetcher`**；**`/api/v1/platform/providers`**：**`product_publish`** **`mock=available`**，**`tiktok`/`shopee`/`lazada`=`beta`**，**`amazon`=`planned`**；类目属性第一版依赖草稿 **`attrs` + `options.lazada_attributes`**；**`product_publications` / `product_publication_skus`**；**`POST /products/:id/publish`** 权限错误 **403**（文案含多平台开放平台）。Amazon **`ErrProductPublishNotImplemented`**。）
...
- **库存同步基础框架**：模块 **`internal/modules/inventory`**；迁移 **`inventory_change_logs`**、**`inventory_sync_tasks`**。**`InventorySyncProvider.SyncInventory`**：**mock** 模拟成功 · **manual** 显式不支持 · **TikTok / Shopee / Lazada / Amazon** 的 **`inventory_sync`** 为 **`planned`**（**`SyncInventory`** 返回未实现类错误）；**`GET /api/v1/platform/providers`** 含 **`capabilityStatus.inventory_sync`**。**队列：** **Redis** **`INVENTORY_SYNC_*`**；Worker **`inventory_sync`**；**`/health`** **`inventorySyncQueue`**；**`/workers/monitor`** **`leasedTasks.inventorySync`**；**TaskReaper** 回收 **`inventory_sync_tasks`**。**JWT**：**`POST /api/v1/products/:id/skus/:skuId/adjust-stock`**（**`inventory.stock.adjust`**；默认更新 **`product_skus`**，可选 **`sync=true`** 建批量任务）；**`POST /api/v1/product-publication-skus/:id/sync-inventory`**、**`POST /api/v1/products/:id/sync-inventory`**；**`GET /api/v1/inventory-sync/tasks`**（含 **retry**）；**`GET /api/v1/products/:id/publication-skus`**；**`GET /api/v1/products/:id/skus/:skuId/inventory-logs`**、**`GET /api/v1/inventory/logs`**；**`inventory.sync.*`** 操作日志（**不写 token**）。成功回填 **`product_publication_skus.stock`** 并记 **`inventory_change_logs`**（含 **`sync_success`** 等）。管理端：**`/inventory/sync-tasks`**、草稿 **「库存」Tab**、**`admin/src/services/inventory.ts`**；**.env.example** 含 **`INVENTORY_SYNC_*`**。
...
27. **TikTok / Shopee / Lazada / Amazon 真实库存写入 API**：库存同步框架（表、队列、Worker；**mock** 可跑通 **`SyncInventory`**）已就位；四平台仍为 **`planned`**，调用返回 **`ErrInventorySyncNotImplemented`**，需在各自 **`providers/platform/*`** 落地真实写入与字段映射。**订单自动扣减 / 回补 **`product_skus`**（**`inventory_change_logs`** 预留 **`order_deduct`/`order_cancel_restore`**）**未实现**。
...
31. **Lazada 商品刊登（beta）边界**：依赖 **`settings.platform_publish_lazada`**（**`default_category_id` / 包裹重量·长宽高 / `delivery_option` / `default_brand_id`（写入 `brand`）** 等）与 **`attrs` + `options.lazada_attributes`** 填报类目必填属性；**`/image/migrate`**（公网图）+ **`/image/upload`**（Storage **Get**）；**`payload` JSON** **`Request.Product`** 随 **Open Platform** 演进需校准；**`publish_as_draft`** 仅落摘要，**不强改平台字段**。
32. **Shopee 商品刊登（beta）边界**：依赖 **`settings.platform_publish_shopee`**（**`default_category_id` / `logistic_channel_id` / `default_weight`** 等）与 **`shop_auth_tokens`**；类目属性由 **`shopee_attribute_list`**（**`attrs` / `options`**）手工提供；多 SKU 默认单维度 **`Variation`**，多维规格用 **`options.shopee_tier_variation`**；**`media_space/upload_image` · `add_item` · `init_tier_variation`** 字段映射随文档升级需校准。

## 8. 下一步开发计划（建议顺序）

1. **TikTok 真实库存写入 API**：在 **`providers/platform/tiktok`** 实现 **`SyncInventory`**（含 **`inventory_sync` capability rollout**）。
2. **Shopee 真实库存写入 API**：同上 **`platform/shopee`**。
3. **Lazada 真实库存写入 API**：同上 **`platform/lazada`**。
4. **Amazon 商品刊登真实 API 接入**（SP-API Listings / Feeds）；随后 **Amazon **`SyncInventory`****。
5. **Amazon SP-API 生产实测与限流优化**（凭证、`429`、多区域 endpoint、Messaging 与 Orders 字段完整性；Buyer-Seller Messaging **模板可用性**随订单状态的实测）。
6. **Collector 反爬与稳定性增强**。
7. **TikTok / Shopee / Lazada / Amazon 订单同步生产实测**：分页、字段、沙箱与 **`cursor`/offset**；按需补 **`docs/platform-provider.md` / `api.md`**。
8. **订单同步深化**：租户隔离、**`external_order_id`/`order_no` 冲突策略**、SKU 与 **`products` 草稿联动**、**`cursor`** 持久化策略。
9. （可选后续）**订单驱动本地库存**：在 **`inventory_change_logs`** 中落地 **`order_deduct`/`order_cancel_restore`** 并联动 **`product_skus.stock`**。
