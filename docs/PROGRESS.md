...
**最后更新**：2026-05-17（**Amazon 真实客服消息 API（beta / Messaging API）**：**`providers/platform/amazon`** 新增 **`customer_messages.go` / `customer_message_api.go` / `customer_message_mapping.go` / `customer_message_paths.go`**；**`PullMessages`**：**Orders v0 分页** + **Messaging `getMessagingActionsForOrder` / `GetAttributes`**（**HAL `application/hal+json`**）；会话 **`external_conversation_id=amazon:{marketplaceId}:{amazonOrderId}`**；消息同步含 **说明性 system 模板摘要**（**SP-API 不提供买家会话正文历史**，避免伪造聊天记录）；**`SendMessage`**：在允许的模板中选 **`confirmOrderDetails` → `confirmDeliveryDetails` → `confirmCustomizationDetails` → `unexpectedProblem`**，POST **`{"text":...}`**（≤2000 runes）；**客户端节流 ~1 rps** 降低 Messaging 限流风险；**403 / Unauthorized** 使用 **`%w` 包装 **`ErrPlatformCustomerMessagePermissionDenied`**（发送可走 **`errors.Is`→403**）；**复用 `settings.platform_amazon` + `shop_auth_tokens` + LWA refresh + `doSPAPIFull` SigV4**；**`CustomerMessageImplementationStatus(amazon)=beta`**；**`GET /platform/providers`**：**Amazon `customer_message`** = **`beta`**；**订单同步不受影响**；管理端 **`/shops`** / **`send-platform-message`** 权限文案补充 **Amazon Buyer-Seller Messaging / Messaging API**）
