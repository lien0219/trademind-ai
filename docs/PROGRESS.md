...
**最后更新**：2026-05-17（**Shopee 真实客服消息 API（beta）**：**`providers/platform/shopee`** 实现 **`CustomerMessageProvider.PullMessages` / `SendMessage`**（**`/api/v2/sellerchat/*`** 路径常量；**复用既有 `postShop`/`getShopWithStatus` 签名与 `shop_auth_tokens` + 刷新**；**不写死 scope**；权限类错误 → **`platform customer message permission denied or not configured`**；可重试错误带 **`retryable`** 提示；**`PullMessages`**：**`get_conversation_list`** → 逐会话 **`get_message`**；**`SendMessage`**：**`get_one_conversation` 解析 `to_id` + `send_message`（`message_type` 文本以 Partner Center 枚举为准）**；**`GET /api/v1/platform/providers`**：**TikTok / Shopee **`customer_message`** = `beta`**；**Lazada / Amazon** 仍为 **`planned`**；**mock / manual** 不变；**管理端 `/shops` / 客服工作台** 与 **`beta`** 对齐；**发送平台消息 403** 为泛化中文权限说明；**订单同步不受影响**；**TikTok 客服消息 beta 不变**）
...
- **迁移**：启动时 `database.AutoMigrate`：在既有 **`admin_users`/`settings`/`operation_logs`/`files`/商品与 SKU/图片任务/订单与同步任务/店铺与授权/采集/worker/`ai_*`/客服相关表** 基础上，**`customer_conversations`** 含可空 **`external_conversation_id`、`raw_data`（JSONB）**；**`customer_messages`** 含 **`external_message_id`、`message_type`（默认 text）、`raw_data`**；新增 **`customer_message_sync_tasks`**（**`task_type=customer_message_sync`**，租约列 **`locked_by`/`locked_until`/`lock_version`**）；其余 **EnsureDefaults / image 默认键** 等逻辑不变（详见前版长列表）。
- **Redis**：`internal/rdb`（go-redis），连接失败 **仅告警**，服务继续（健康检查体现 `redis: skipped/degraded`）。
- **健康检查**：`GET /health`、`GET /api/v1/health`（含 DB/Redis 检查；**`data.collectQueue`** …；**`data.imageQueue`** …；**`data.orderSyncQueue`** …；**`data.customerMessageSyncQueue`** …；**`data.workers`**：**`heartbeatEnabled`/`reaperEnabled`、按 `running`/`stale`（心跳推导）汇总、`byType`（`collect`/`image`/`order_sync`/`customer_message_sync`）**；Worker 查询失败时 **`workers.degraded`** 且整体可 **`degraded`**；**不对 Collector 做 HTTP 探测**以免拖慢健康接口）。队列开启且 Redis **Ping 正常但 LLEN 不可得**时整体 **`status` 可标记 `degraded`**。
...
- **平台客服消息同步（框架 + TikTok / Shopee 真实 API beta）**：模块 **`internal/modules/customersync`**；表 **`customer_message_sync_tasks`**（租约 + **`input`/`output` JSONB**，**不落 token**）；**`internal/providers/platform.CustomerMessageProvider`**（**`PullMessages`/`SendMessage`**；**mock** 可拉取/发送仿真消息；**TikTok** **`providers/platform/tiktok`**、**Shopee** **`providers/platform/shopee`** 真实调用平台客服 / Seller Chat API（响应字段 **按官方文档演进时需调整映射**）；**lazada/amazon** 骨架仍返回 **`platform customer message provider not implemented`**；**manual** 不支持）；**`customerchat.SyncPlatformCustomerMessages`** 幂等 upsert **`customer_conversations`/`customer_messages`**（裁剪 **`raw_data`**）；**`POST /api/v1/shops/:id/sync-customer-messages`**、**`GET /api/v1/customer/message-sync/tasks`**、**`GET .../:id`**、**`POST .../:id/retry`**；Redis **`CUSTOMER_MESSAGE_SYNC_*`** + Worker **`customer_message_sync`**（队列关闭时可同步执行）；**`/health` `customerMessageSyncQueue`**；**TaskReaper** 回收 **`customer_message_sync_tasks`** 超时 **`running`**；**`POST /api/v1/customer/conversations/:id/send-platform-message`**：**仅人工**触发外发，成功后 **`source=platform`** 写 **`customer_messages`**，**`accept` 建议仍仅内部记录**；权限不足时 **`platform customer message permission denied or not configured`**（HTTP **发送**接口 **403** 附中文说明）。
...
- **多实例 Worker 与任务租约（MVP）**：模块 **`internal/modules/worker`**（**`worker_instances`**：`worker_id`/`worker_type`（**`collect`/`image`/`order_sync`/`customer_message_sync`**）/`status`（`running`/`stale`/`stopped`）/心跳；**`WORKER_HEARTBEAT_*` / `WORKER_STALE_*` / `WORKER_REAPER_*` / `WORKER_LEGACY_RUNNING_TIMEOUT_SECONDS`**）；**`internal/modules/taskreaper`** 定时回收 **`locked_until`** 到期 **`running`**（采集走现有自动重试/失败；图片走 **`ErrWorkerLeaseExpired`** 与退避；订单同步 **直接 failed**+**`order.sync.lease_expired`**；**客服消息同步任务** 同订单同步策略回收租约）；**legacy**：**`running` 且 `locked_by` 空**且 **`updated_at` 过旧**。**JWT**：**`GET /api/v1/workers/monitor`**（**`leasedTasks.customerMessageSync`**）。管理端：**`/workers/monitor`**。操作日志：**`worker.instance.start`/`stop`**；采集事件 **`worker.lease.acquired`/`expired`/`recovered`**（及既有 **`task.*`**）；图片 **`image.task.lease_expired`**。
...
19. **AI 客服与内部订单上下文**：工作台已支持 **绑定内部订单**（含 **同步入库的 `orders`**）。**已落地** **平台客服消息同步**（**`customersync`**、**人工外发**、**mock** 全链路可测）；**TikTok**、**Shopee** **`PullMessages`/`SendMessage` 已真实接入（`beta`**；**TikTok** 依赖 **`api_version`** 与 Customer Service 路径；**Shopee** 依赖 **`api/v2/sellerchat/*`**，**`SendMessage` 前 `get_one_conversation` 解析买家 `to_id`**；字段映射均可随 Open Platform 文档继续校准）；**Lazada / Amazon** 仍为占位。**`GET /platform/providers`**：**TikTok / Shopee `customer_message`** 为 **`beta`**。仍无各平台 **实时消息 WebSocket 推送**。
...
33. **Lazada / Amazon 真实客服消息 API**：`PullMessages` / `SendMessage` **未接入**（**骨架返回未实现**；**订单同步不受影响**）。**TikTok / Shopee** 已接 **`beta`**（见 §3.2 客服同步条）。
34. **平台商品刊登未完成**（多平台）。
35. **库存同步未完成**（多平台）。

## 8. 下一步开发计划（建议顺序）

1. **Lazada / Amazon 真实客服消息 API 接入**（在 **`providers/platform/lazada|amazon`** 内实现 **`PullMessages`/`SendMessage`**；**TikTok / Shopee** 已完成 **beta**）。
2. **平台商品刊登**（多平台，与订单管道解耦）。
3. **库存同步**（多平台，独立于本里程碑）。
4. **Amazon SP-API 生产实测与限流优化**（凭证、`429`、多区域 endpoint、字段完整性）。
5. **Collector 反爬与稳定性增强**。
6. **TikTok / Shopee / Lazada 订单同步生产实测**：分页、字段、沙箱与 **`cursor`/offset**；按需补 **`docs/platform-provider.md` / `api.md`**。
