...
**最后更新**：2026-05-16（**第三方服务配置中心统一整理**：静态 **`IntegrationConfigDefinitions`** + **`GET /api/v1/settings/integration-schemas`**；**`GET /api/v1/settings/integrations/overview`** 汇总 AI / 图片 / 存储 / **mail（合并 legacy `email`）** / 各 **`platform_*`** / **`collect_rules` 数量**；**SMTP**：**`PlainMailSettings`**（`mail` 优先覆盖 `email`）、**注册验证码 / test-email** 走合并读取；**补齐 `POST /api/v1/settings/test-email` 路由**；**`/settings/integrations`** 总览页；设置菜单顺序：**系统 → 邮箱 → AI → 图片 AI → 存储 → 平台开放 → 采集 → 安全**；**平台 PUT**：**`planned`** 平台允许**部分保存**（不强制必填校验）；**`GET/PUT /platform/settings/:platform`** 响应含 **`schema`**；店铺**创建 / 授权 Drawer** 校验 **`appConfigSchema` 必填**并提示跳转平台配置；**AI 设置** 增加 **temperature / max_tokens** 与说明 **Alert** — §3/§7）
...
- **店铺管理（授权基座）**：路由 **`/shops`**；**`services/shops.ts`** + **`platformOpen.ts`**：**TikTok OAuth**；默认 **不写每店 App Key/Secret**，引导 **「设置 → 平台开放配置」**；**`formatPlatformPartnerErr`** 映射 **`platform config incomplete`**、**`required setting missing`** 等；**创建店铺 / 打开授权 Drawer** 前 **`GET /api/v1/platform/settings/:platform`** 校验 **`appConfigSchema` 必填项**（**`****` 视为已配置密钥**），缺省提示 **先到平台开放配置**；**生成 TikTok 授权链接** 在配置不完整时 **二次 warning**。**同步订单 Modal**、`/orders/sync-tasks`。**`SHOP_*` / `PLATFORM_PROVIDER_STATUS`**。
