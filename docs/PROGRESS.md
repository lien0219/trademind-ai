...
**最后更新**：2026-05-16（**统一店铺 `shops` + `shop_auth_tokens`（MVP 基座，无真实 OAuth）**：**`internal/providers/platform`** 注册表（**`manual` / `mock`：`available`**；TikTok·Shopee·Lazada·Amazon·AliExpress·Shopify·WooCommerce·eBay·Temu·SHEIN·**`custom`：`planned`**）；**JWT** **`GET /api/v1/platform/providers`**、**`GET/POST/PUT/DELETE /api/v1/shops`**、**`PUT …/auth`**（敏感字段 **AES-GCM + 脱敏 + `****` 不覆盖**）、**`POST …/test-connection`**（**manual/mock** 占位成功，**planned** 明确未实现）；**`orders.shop_id`**、**`customer_conversations.shop_id`** + 列表/详情 **`shopSummary`**；管理端 **`/shops`**、订单 **`shopId`**、客服会话/详情 **关联店铺 Modal**。**自定义链接采集器（Collector `beta`）**：表 **`collect_rules`** + **`collect_tasks.request_options`**；**JWT API** **`GET/POST/PUT/DELETE …/collect/rules`**、**`…/enable|disable|test`**（测试走 Collector **`POST /v1/collect`** + **`options.rule`**，不落任务/草稿）；**Collector `sourceCustom`**（CSS Selector + **JSON-LD / OpenGraph / Meta** fallback，**NormalizedProduct** / **`raw.stateDigest`**）；**`POST /collect/tasks`** 支持 **`source=custom`** + **`ruleId`** 或域名自动匹配启用规则；Worker **快照 rule** 下发 Collector（**不写 PDP 解析进 Go**）；管理端 **`/collect/rules`**、**`/collect/tasks`** custom **规则下拉**；Hub **custom 批量**提示「自定义采集器暂不支持批量」。其它：**AliExpress beta**、1688 **available**；**拼多多 / 淘宝 / SHEIN·Temu** 仍为 **`planned`**。）
...
- **迁移**：启动时 `database.AutoMigrate` — `admin_users`、`settings`、**`operation_logs`**、**`files`**、**`products`、`product_images`、`product_skus`、`image_tasks`**、**`orders`、`order_items`、`order_shipments`**、**`shops`、`shop_auth_tokens`**、`collect_batches`、`collect_tasks`、`collect_task_events`、**`collect_rules`**、**`ai_prompts`、`ai_tasks`**、**`customer_conversations`**（含可空 **`order_id`**、**`shop_id`**）、**`customer_messages`、`customer_reply_suggestions`**；启动后 **`aiprompt.EnsureDefaults`**（含 **`migrateCustomerReplyGenerateOrderContext`** 对存量 **`customer_reply_generate`** 占位符升级）、写入默认 **`product_title_optimize`**、**`product_description_generate`**、**`customer_reply_generate`**（各仅在缺失时插入）；**`settings.EnsureImageDefaults`** 写入 **`image` 分组**默认键（**`provider=noop`**；**`removebg_*`**；**`openai_image_*`**；**`comfyui_base_url`（`http://127.0.0.1:8188`）**、**`comfyui_api_key`（加密，可选）**、**`comfyui_workflow_json`（json，空）**、**`comfyui_prompt_node_id` / `comfyui_image_node_id` / `comfyui_output_node_id`**、**`comfyui_timeout_sec`（180）**、**`comfyui_poll_interval_seconds`（2）**、**`comfyui_max_poll_seconds`（180）**；**`timeout_sec`**；**仅缺失键插入**）
...
- **内部订单**：路由 **`/orders`**；**ProTable + Drawer**：头表表单（含可选 **`shopId`**）、Tabs **明细行（`order_items`）与发货（`order_shipments`）** 及独立新增/编辑 Modal；状态 Tag 映射 **`ORDER_*`**；**`services/orders.ts`**。
- **店铺管理（授权基座）**：路由 **`/shops`**（侧栏 **店铺 → 店铺管理**）；**`services/shops.ts`**：**平台 Provider 列表**、店铺 CRUD、**授权表单（敏感项脱敏）**、**测试连接**；常量 **`SHOP_*` / `PLATFORM_PROVIDER_STATUS`**（`status.ts`）。
...
- **常量**：`src/constants/status.ts`（商品状态、**采集任务 / 批次**状态枚举、**订单与支付 / 履约 / 发货**、**店铺与平台 Provider**）。
...
│       └── modules/         # auth、admin、settings、**operationlog**、**files**、**product**、**order**、**collect**、**collectrule**、**aiprompt**、**aitask**、**imagetask**、**customerchat**
...
4. **Collector 演进（补强 / 扩展）**：**AliExpress**：**批量链路开放前**，补强 **SKU 稳定性与异步详情**。**新源**：在 **`pdd` / `taobao` / `shein_temu`** 中选 **其一**落地 **真实结构化解析**（**对齐 `NormalizedProduct`**）。**自定义**：**规则可视化编辑器**、**批量开放（若需）**、**SSRF 深度防护**。反爬：**SKU 多维、异步详情 iframe、人机分流**（**仅限 Collector**，Go **不写 PDP 解析**）。
...
| 日期 | 说明 |
|------|------|
| 2026-05-16 | **统一店铺 `shops`（平台授权基座，无 OAuth）**：**`shops`/`shop_auth_tokens`** + **`internal/providers/platform`**（**manual/mock `available`**，多平台 **`planned`**）；**`/api/v1/platform/providers`**、**`/api/v1/shops*`**（**auth 加密脱敏**、**test-connection**）；**`orders`/`customer_conversations` `shop_id` + `shopSummary`**；管理端 **`/shops`**、订单/会话/详情 **店铺关联**；**客服详情「选择店铺」Modal**；**PROGRESS** 对齐 |
