...
完整 ERP 按可验收闭环渐进建设，详细边界见 [ERP 扩展架构](ERP_ARCHITECTURE.md)。当前第一阶段已完成仓库、供应商、采购单审批、部分收货和仓库库存流水的后端基础；管理端工作台和全库存写路径迁移仍待完成。

后续能力顺序：

- Admin 采购与仓库工作台
- 人工调库、订单预占/扣减/释放的分仓迁移
- 采购退货、库存调拨与盘点（调拨与盘点首版已进入 `dev`，等待 CI 与人工验收）
- 复杂售后 / 退款
- 复杂财务结算
- WMS / OMS
- 复杂 BI
