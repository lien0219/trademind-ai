...
完整 ERP 按可验收闭环渐进建设，详细边界见 [ERP 扩展架构](ERP_ARCHITECTURE.md)。当前已完成仓库/供应商主数据、采购单与分批收货、Admin 采购工作台、人工调库和历史库存迁移/对账、订单库存生命周期、仓库调拨、库存盘点、采购退货、只读安全库存/补货建议 V1、订单明细级销售售后/退货退款 V1，以及本地平台结算 CSV 导入、订单费用对账、履约成本快照 V1 和订单预估利润 V3 的代码闭环；新实现仍需现有 CI、隔离 PostgreSQL 与人工业务验收后才能签收。
...
- 持续验收销售售后的累计可退、仅退款零库存写、良品/残次品收货和职责分离边界
- 持续验收结算 CSV 的 PostgreSQL 并发幂等、不可变交易、店铺隔离和费用对账边界
- 平台售后同步、实际支付退款、换货，以及真实结算 Provider 的独立评审
- 广告/仓储费用账、应收应付、会计凭证与复杂财务结算
- WMS / OMS
- 复杂 BI
- 自动化规则引擎
- 自动补货
