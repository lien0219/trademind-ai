...
完整 ERP 按可验收闭环渐进建设，详细边界见 [ERP 扩展架构](ERP_ARCHITECTURE.md)。当前已完成仓库/供应商主数据、采购单与分批收货、Admin 采购工作台、人工调库和历史库存迁移/对账、订单库存生命周期、仓库调拨、库存盘点及采购退货的代码闭环；新实现仍需现有 CI 与人工业务验收后才能签收。

后续能力顺序：

- 持续验收订单库存、调拨、盘点与采购退货的并发和对账边界
- 收敛剩余旧库存写入，将 `product_skus.stock` 明确降级为仓库余额兼容投影
- 安全库存与补货建议（首版只读，不自动建采购单、不启动新 Worker）
- 复杂售后 / 退款
- 复杂财务结算
- WMS / OMS
- 复杂 BI
