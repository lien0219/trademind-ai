...
完整 ERP 按可验收闭环渐进建设，详细边界见 [ERP 扩展架构](ERP_ARCHITECTURE.md)。当前已完成仓库/供应商主数据、采购单与分批收货、Admin 采购工作台、人工调库和历史库存迁移/对账、订单库存生命周期、仓库调拨、库存盘点、采购退货、只读安全库存/补货建议 V1，以及订单明细级销售售后/退货退款 V1 的代码闭环；新实现仍需现有 CI 与人工业务验收后才能签收。
