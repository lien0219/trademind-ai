...
| 店铺授权 | 抖店 OAuth 授权闭环、state 防护、token 加密保存、刷新 / 解除 / 测试连接；TikTok Shop / Shopee / Lazada / Amazon 授权基座保留 | 抖店 Phase 2 已支持 |
...
多平台跨境 ERP MVP
├── 真实平台闭环：抖店优先（OAuth 授权已打通 → API Client / 签名层 → 类目属性 → 图片上传 → 商品草稿 → 订单同步 → 库存同步）
