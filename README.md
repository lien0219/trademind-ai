...
- 平台结算、仓库操作费与订单预估利润：支持租户/店铺范围内的本地 CSV 校验预览、明确确认和不可变结算交易导入，按订单展示一致、待核对、存在差异或暂不可核对的结果并导出；按仓库维护版本化出库、拣货和打包费率，对已完成且通过打包扫描复核的订单先预览再人工确认不可变费用快照，更正只追加调整或一次性冲正；订单预估利润 V4 对已履约订单使用发货时冻结的不可变商品成本和已确认仓库操作费，历史缺口不以当前价格回填，广告费用仍保持缺失，所有快照均不冒充会计实际成本。
