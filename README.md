...
| 模块 | 能力 | 当前状态 |
| --- | --- | --- |
| 商品采集 | 1688 可用采集、AliExpress beta、淘宝/天猫 beta、拼多多可用、自定义规则 beta、采集任务与批次 | 已支持 |
...
适合从 **淘宝 / 天猫商品详情页** 采集商品草稿（`item.taobao.com`、`detail.tmall.com`、`detail.tmall.hk`、`world.taobao.com`）。

1. **启动采集服务**：本地执行 `pnpm dev:collector`；管理端 **设置 → 采集设置 → 淘宝/天猫** 确认采集服务地址与超时、重试配置。
2. **登录（如需要）**：点击 **打开淘宝/天猫采集浏览器**，在弹出窗口中完成登录或安全验证；可填写 **用于检测的商品链接** 后点 **重新检测**。
3. **单链接采集**：在 **采集中心** 选择淘宝/天猫采集器，粘贴商品详情链接并提交；完成后在 **商品草稿** 中核对标题、价格、主图、规格与库存。
4. **批量采集**：当前 **暂未开放**。

| 常见问题 | 建议操作 |
| --- | --- |
| 需要登录 | 打开淘宝/天猫采集浏览器登录后重试 |
| 需要验证 | 在采集浏览器中手动完成滑块/验证码 |
| 主图为空 | 任务失败，请确认页面加载完整后重试 |
| 价格/规格不完整 | 草稿可创建，发布前人工复核 |

**说明**：Beta 阶段 SKU / 库存 / 详情图可能不完整；自定义链接采集器 **不会** 处理上述域名（会提示使用专用采集器）。
