...
| 店铺授权 | 抖店 OAuth 授权闭环、API Client / 签名层、token 加密保存与自动刷新、刷新 / 解除 / 真实测试连接、类目 / 属性缓存；TikTok Shop / Shopee / Lazada / Amazon 授权基座保留 | 抖店 Phase 4 已支持 |
| 多平台配置 | 平台开放配置 Schema、抖店配置、Service ID、敏感配置加密与脱敏 | 已支持 |
| 订单同步 | 多平台订单同步框架、任务队列、异常工作台 | 开发中 |
| 商品刊登 | 定价规则、图片同步、发布前检查、平台字段映射、刊登任务、失败中心 | 生产级完善中 |
...
多平台跨境 ERP MVP
├── 真实平台闭环：抖店优先（OAuth 授权、API Client / 签名层、类目 / 属性缓存已打通 → 字段映射与图片上传 → 商品草稿 → 订单同步 → 库存同步）
...
2. **第二优先级：多平台跨境 ERP MVP**
   - 先把抖店真实平台闭环跑通：店铺授权、类目属性缓存已完成；下一阶段进入商品字段映射与图片上传，再继续商品草稿创建、订单同步、库存同步；TikTok Shop / Shopee / Lazada / Amazon 作为后续平台接入，不多平台并行铺开。
3. **后续迭代：完整 ERP 增强**
