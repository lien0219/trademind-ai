...
1. **启动采集服务**：本地执行 `pnpm dev:collector`；管理端 **设置 → 采集服务 → 淘宝/天猫** 配置超时、重试、详情图等待、SKU 点击采集等。
2. **登录（如需要）**：点击 **打开淘宝/天猫采集浏览器**（独立登录浏览器，不与 1688/拼多多混用），完成登录或安全验证后点 **重新检测**。
3. **单链接采集**：在 **采集中心** 选择淘宝/天猫采集器（状态 **测试中**），粘贴商品详情链接并提交。
4. **核对草稿**：商品详情页顶部会提示采集风险；价格/规格/详情图不完整时会有 warning，发布前检查会拦截缺价。
5. **同步图片（可选）**：商品草稿 **图片管理** 可将外链主图/详情图同步到当前存储设置（本地/COS/OSS/S3/R2/MinIO）。
6. **批量采集**：当前 **暂未开放**，请先使用单个采集。
