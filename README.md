...
- 商品刊登：支持多平台刊登中心（单商品多店铺预检查与批量创建刊登草稿）、**多商品批量刊登向导**（商品列表多选 → 批次与子任务；Phase A2.2 含生产级统一配置 / 单独覆盖 UI、生效配置预览与配置校验；A2.1 含批量上限、显式 migration、幂等测试与生产收口）、**批量 AI 标题/描述**（Phase A3.1：四步向导 → 复核工作台 → 冲突保护应用与撤销）、刊登草稿映射、发布任务、失败回收与人工校正；抖店可创建真实平台草稿，其他平台当前以本地草稿快照为主。
