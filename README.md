...
<p align="center">
  <strong>AI First Cross-border E-commerce Operation Platform</strong>
</p>

<p align="center">
  开源 AI 跨境电商运营平台 · 商品采集 · AI 商品优化 · 多平台店铺管理 · 跨境 ERP 能力扩展
</p>
...
<p align="center">
  <a href="#贸灵-trademind">简体中文</a> | <a href="#english">English</a>
</p>

> **贸灵 TradeMind** 是一个面向跨境电商卖家的开源 AI 运营工具，目标是先做好 AI 商品运营闭环，再逐步扩展为多平台跨境 ERP。

## 项目截图 / Demo

> Demo 与截图即将补充。欢迎提交 PR 帮助完善首页展示素材。
...
贸灵 TradeMind 是一个面向跨境电商卖家的开源 AI 运营平台。它不是一开始就追求“大而全”的传统 ERP，而是优先完成最有价值、最容易产生效率提升的 AI 商品运营链路：

```text
商品采集 → 商品草稿 → AI 标题优化 → AI 描述生成 → 图片管理
  → 店铺授权 → 商品刊登 → 订单同步 → AI 客服
```

当前项目的第一优先级是 **AI 商品运营工具**，第二优先级是 **多平台跨境 ERP MVP**，后续再逐步增强完整 ERP 能力。

## 为什么做这个项目

跨境卖家的日常运营中存在大量重复工作：采集商品、整理标题、生成多语言描述、处理商品图、维护平台店铺、同步订单、回复买家消息。传统 ERP 解决了数据管理问题，但很少真正把 AI 嵌入商品运营流程。

TradeMind 希望提供一个开源、可部署、可二次开发的基础平台，让个人卖家、团队和开发者都能围绕自己的业务流程接入 AI Provider、Storage Provider、Collector Provider 与 Platform Provider。

## 核心功能

| 模块 | 能力 | 当前状态 |
| --- | --- | --- |
| 商品采集 | 1688 可用采集、AliExpress / 自定义规则 beta、采集任务与批次 | 已支持 |
...
| 自动化运营 | 失败任务中心、告警、批量 AI、刊登 / 库存同步基础 | 预留架构 |
...
适合想快速跑起完整项目的使用者。仓库已包含 `docker-compose.full.yml`、`backend/Dockerfile`、`admin/Dockerfile`、`collector/Dockerfile` 与 `admin/nginx.conf`。
...
TradeMind is an open-source AI commerce operation platform for cross-border sellers. It focuses first on the most valuable AI product operation workflow instead of building a heavy traditional ERP from day one.

```text
Product collection → Product drafts → AI title optimization → AI description generation
  → Image management → Store authorization → Product publishing
  → Order sync → AI customer service
```

The first priority is an **AI product operation tool**. The second priority is a **multi-platform cross-border ERP MVP**. Full ERP capabilities will be added gradually.

## Features

| Module | Capability | Status |
| --- | --- | --- |
| Product Collection | 1688 collection, AliExpress beta, custom rule collector | Supported |
| Product Drafts | Products, SKUs, images, inventory thresholds, readiness check | Supported |
| AI Title Optimization | OpenAI-compatible provider, prompt templates, task records | Supported |
| AI Description Generation | Product description generation and task tracking | Supported |
| SKU Candidate Recommendation | Candidate SKUs for order items and manual binding | Supported |
| Image Management | Local / cloud storage, file upload, product images | Supported |
| AI Image Processing | remove.bg, OpenAI Image, ComfyUI, async image tasks | Supported |
| Store Authorization | TikTok Shop, Shopee, Lazada, Amazon foundation | In progress |
| Platform Configuration | Provider schema, encrypted sensitive settings | Supported |
| Order Sync | Multi-platform order sync framework and exception workspace | In progress |
| AI Customer Service | Message sync, AI suggested replies, manual send confirmation | In progress |
| Automation | Task center, alerts, batch AI, publish / inventory task foundation | Architecture reserved |
...
Before production deployment, update `JWT_SECRET`, `APP_MASTER_KEY`, `ADMIN_BOOTSTRAP_PASSWORD`, database passwords, and all other sensitive values.
...
TradeMind uses provider abstractions for AI, storage, image processing, platform integrations, and collectors.

## Roadmap

- v0.1.0: foundation, login, settings, local storage, Docker
- v0.2.0: AI text capabilities and prompt templates
- v0.3.0: product drafts, SKUs, images
- v0.4.0: collector service and collection tasks
- v0.5.0: AI image processing
- v0.6.0: store authorization, order sync, product publishing, inventory sync
- v0.7.0: AI customer service preview
...
Contributions are welcome. You can report bugs, suggest features, improve documentation, add providers, optimize collector rules, improve prompt templates, or improve Docker deployment.

Please read [CONTRIBUTING.md](CONTRIBUTING.md) before opening a PR.

## Open Source Usage

This project is licensed under Apache-2.0. You may use, modify, distribute, and commercialize it, but you must keep the original license and clearly mention the original project repository in your derived work.
...
TradeMind is licensed under the [Apache License 2.0](LICENSE).
