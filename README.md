...
| 店铺授权 | 抖店 OAuth 授权闭环、API Client / 签名层、token 加密保存与自动刷新、刷新 / 解除 / 真实测试连接；TikTok Shop / Shopee / Lazada / Amazon 授权基座保留 | 抖店 Phase 3 已支持 |
...
多平台跨境 ERP MVP
├── 真实平台闭环：抖店优先（OAuth 授权与 API Client / 签名层已打通 → 类目属性 → 图片上传 → 商品草稿 → 订单同步 → 库存同步）
...
2. **第二优先级：多平台跨境 ERP MVP**
   - 先把抖店真实平台闭环跑通：店铺授权、类目属性、图片上传、商品草稿创建、订单同步、库存同步；TikTok Shop / Shopee / Lazada / Amazon 作为后续平台接入，不多平台并行铺开。
3. **后续迭代：完整 ERP 增强**
