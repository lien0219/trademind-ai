...
- 平台结算与订单预估利润：支持租户/店铺范围内的本地 CSV 校验预览、明确确认和不可变结算交易导入，按订单展示一致、待核对、存在差异或暂不可核对的结果并导出；订单预估利润 V3 对已履约订单使用发货时冻结的不可变成本快照，历史快照缺失时不以当前价格回填，未履约订单仅使用明确标注的目录估算；广告和仓储费用仍保持缺失，快照不冒充会计实际成本。
