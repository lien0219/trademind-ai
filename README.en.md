...
- Store-scoped local settlement CSV validation, explicit confirmation, immutable transaction import, and bounded order reconciliation export. Versioned warehouse rate cards cover outbound, per-item picking, and per-package packing fees; only completed, packing-verified orders can be previewed and explicitly confirmed into immutable snapshots, with corrections recorded as append-only adjustments or one-time reversals. Order profit estimate V4 uses immutable fulfillment-time product costs and valid confirmed warehouse-operation fees, never backfills historical gaps with current prices, and keeps advertising fees unset. These snapshots are estimates and operational facts, not accounting COGS.
