...
- Store-scoped local settlement CSV validation, explicit confirmation, immutable transaction import, and order reconciliation that clearly distinguishes consistent results, items awaiting review, discrepancies, and unavailable comparisons, with bounded export. Order profit estimate V3 uses immutable fulfillment-time cost snapshots for fulfilled orders, never backfills historical snapshot gaps with current prices, and labels current catalog costs as estimates only for unfulfilled orders. Advertising and warehouse fees remain unfilled, and snapshots are not presented as accounting COGS.
