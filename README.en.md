...
- Store-scoped local settlement CSV validation, explicit confirmation, immutable transaction import, and `matched` / `pending` / `mismatch` / `blocked` order reconciliation with bounded export. Only matched platform fees feed order profit estimate V2; advertising and warehouse fees remain unfilled, and current supplier prices are not presented as historical COGS.
