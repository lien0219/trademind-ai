...
- Store-scoped local settlement CSV validation, explicit confirmation, immutable transaction import, and order reconciliation that clearly distinguishes consistent results, items awaiting review, discrepancies, and unavailable comparisons, with bounded export. Only reconciled platform fees feed order profit estimate V2; advertising and warehouse fees remain unfilled, and current supplier prices are not presented as historical COGS.
