...
import { Alert, Button, Form, Input, InputNumber, Select, Space, Spin, Switch, Tabs, Tag, Typography, message } from 'antd';
      ...
      <Space direction="vertical" size={12} style={{ width: '100%' }}>
        {meta.status === 'planned' && (
          <Alert
            showIcon
            type="warning"
            message="该平台刊登能力规划中，可先保存刊登侧默认参数，真实对接前不会调用平台上架 API。"
          />
        )}
        {(schema?.description?.length ?? 0) > 0 && (
          <Typography.Paragraph type="secondary">{schema?.description}</Typography.Paragraph>
        )}
        <Typography.Paragraph type="secondary">
          运行时：<Tag color={st.color}>{st.label}</Tag>；
          Settings 分组 <Typography.Text code>{meta.publishSettingsGroupKey || schema?.groupKey}</Typography.Text>
